...
export type ArtifactDestructionResult = Readonly<{ outcome: "deleted" | "missing" | "tampered"; bytesDestroyed: true }>;
...
export class LocalArtifactStore implements ArtifactStore {
  ...
  async put(storageKey: string, content: Uint8Array, proof: VeilEvidenceAdmissionProof): Promise<AdmittedArtifact> {
    ...
    return {
      ...
      admission: Object.freeze({ manifest: admission, ...(proof.sanitation ? { sanitation: proof.sanitation } : {}), token: proof.token }),
    };
  }
  ...
  async getRange(storageKey: string, offset: number, length: number, rawAdmission: VeilEvidenceAdmissionProof) {
  ...
  async destroy(storageKey: string, admission: VeilEvidenceAdmissionProof): Promise<ArtifactDestructionResult> {
    ...
    try { await this.verifyStored(storageKey, admission); }
    catch { outcome = "tampered"; }
