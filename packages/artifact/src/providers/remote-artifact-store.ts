...
export class RemoteArtifactStore implements ArtifactStore {
  constructor(
    private readonly backend: RemoteObjectBackend,
  ...
  async put(storageKey: string, content: Uint8Array, proof: VeilEvidenceAdmissionProof) {
    ...
    await this.backend.putObject(this.key(storageKey), content, {
      "scry-sha256": checksumSha256,
      "scry-evidence-id": admission.evidenceId,
    });
    return {
      storageKey,
  ...
  async get(storageKey: string, proof: VeilEvidenceAdmissionProof) {
    const content = await this.backend.getObject(this.key(storageKey));
  ...
  async getRange(
    ...
    const metadata = await this.verifyMetadata(storageKey, proof);
    if (length === 0 || offset >= metadata.sizeBytes) return new Uint8Array();
    return this.backend.getObject(this.key(storageKey), {
      offset,
      length: Math.min(length, metadata.sizeBytes - offset),
    });
  }
  async size(storageKey: string, proof: VeilEvidenceAdmissionProof) {
    return (await this.verifyMetadata(storageKey, proof)).sizeBytes;
  }
  async exists(storageKey: string) {
    return this.backend.existsObject(this.key(storageKey));
  }
  async delete(storageKey: string) {
    await this.backend.deleteObject(this.key(storageKey));
  ...
  async destroy(
    ...
    try {
      requireAdmission(
        proof,
        await this.backend.getObject(this.key(storageKey)),
        this.admissionKey,
      );
    } catch {
      outcome = "tampered";
  ...
  private async verifyMetadata(storageKey: string, proof: VeilEvidenceAdmissionProof) {
    const admission = requireAdmissionProof(proof, this.admissionKey);
    const metadata = await this.backend.headObject(this.key(storageKey));
    if (!metadata.checksumSha256 || metadata.checksumSha256 !== admission.contentDigest)
      throw new Error("Stored artifact checksum metadata does not match Veil admission");
    return metadata;
  }
  private key(storageKey: string) {
    return prefixedStorageKey(this.prefix, storageKey);
  }
}
