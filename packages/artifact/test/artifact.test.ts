...
describe("LocalArtifactStore", () => {
  it("stores bytes with stable metadata", async () => {
    ...
    expect(new TextDecoder().decode(await store.getRange("run/trace.zip", 2, 4, proof))).toBe("iden");
  });

  it("rejects traversal keys", async () => {
    const store = new LocalArtifactStore(await mkdtemp(path.join(tmpdir(), "scry-artifact-")), key);
    await expect(store.put("../secret", bytes, signVeilEvidenceAdmission(admission, key))).rejects.toThrow("safe relative");
  });
  ...
  it("destroys valid, missing, and tampered bytes idempotently", async () => {
    ...
    await expect(store.destroy("run/retained.bin", proof)).resolves.toEqual({ outcome: "deleted", bytesDestroyed: true });
    await expect(store.destroy("run/retained.bin", proof)).resolves.toEqual({ outcome: "missing", bytesDestroyed: true });
    await store.put("run/tampered.bin", bytes, proof);
    await writeFile(path.join(root, "run/tampered.bin"), "unsafe replacement");
    await expect(store.destroy("run/tampered.bin", proof)).resolves.toEqual({ outcome: "tampered", bytesDestroyed: true });
  ...
  it("refuses bytes without a matching immutable admission", async () => {
    const store = new LocalArtifactStore(await mkdtemp(path.join(tmpdir(), "scry-artifact-")), key);
    await expect(store.put("run/tampered.bin", new TextEncoder().encode("tampered"), signVeilEvidenceAdmission(admission, key)))
      .rejects.toMatchObject({ code: "VEIL_EVIDENCE_DIGEST_MISMATCH" });
    const quarantined = { ...admission, disposition: "quarantine" as const };
    await expect(store.put("run/quarantine.bin", bytes, signVeilEvidenceAdmission(quarantined, key)))
      .rejects.toMatchObject({ code: "VEIL_EVIDENCE_NOT_ADMITTED" });
    await expect(store.put("run/forged.bin", bytes, { manifest: admission, token: "forged" }))
      .rejects.toMatchObject({ code: "VEIL_EVIDENCE_NOT_ADMITTED" });
    const signed = signVeilEvidenceAdmission(admission, key, { stage: "pre_capture", method: "mask" });
    await expect(store.put("run/tampered-attestation.bin", bytes, { ...signed, sanitation: { stage: "post_capture", method: "mask" } }))
      .rejects.toMatchObject({ code: "VEIL_EVIDENCE_NOT_ADMITTED" });
  });
});
