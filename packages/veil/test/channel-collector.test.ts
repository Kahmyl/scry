...
import { VeilChannelCollector } from "../src/channel-collector.js";
...
describe("VeilChannelCollector", () => {
  it("acknowledges only after the capture fence changes state", async () => {
    ...
    expect(collector.state()).toMatchObject({
      status: "prepared",
  ...
  it("fails closed on invalid transitions and remains suppressed after sealing", async () => {
    ...
    await expect(
      collector.resume({ kind: "known_secret_registered", referenceType: "vault" }),
    ).rejects.toThrow("VEIL_COLLECTOR_NOT_ISOLATED");
