...
import {
  VeilRuntimeCoordinator,
  PrivacyTransitionError,
  type PrivacyCollector,
...
const boundary = {
  kind: "protected_surface_closed" as const,
  verifier: {
    concept: "Credential dialog closed",
    requiredCapabilities: ["readable_value" as const],
    preferredEvidence: {
      roles: ["dialog" as const],
      names: ["Credential dialog"],
      labels: [],
      descriptions: [],
      placeholders: [],
      inputTypes: [],
...
describe("VeilRuntimeCoordinator", () => {
  it("requires every collector before reveal and resumes only after a safe boundary", async () => {
    ...
    const states: string[] = [];
    const gate = new VeilRuntimeCoordinator([recording, trace], async ({ state }) => {
      states.push(state);
    });
  ...
  it("refuses a false suspend acknowledgement and seals all collectors", async () => {
    ...
    await expect(gate.prepare("synthetic")).rejects.toThrow(
      "COLLECTOR_ARM_FAILED:Collectors failed suspend: liar:TRANSITION_FAILED",
    );
    expect(gate.state()).toBe("sealed");
