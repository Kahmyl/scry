...
/** Veil is the sole owner of privacy-policy compilation. */
...
const PROFILES: Readonly<Record<Exclude<VeilProfile, "custom">, Readonly<VeilPolicyControls>>> =
  Object.freeze({
...
export function compileVeilPolicy(
  input: VeilPolicyPreferences | readonly VeilPolicyPreferences[],
): VeilPolicySnapshot {
  const layers = (Array.isArray(input) ? input : [input]).map((layer) =>
    veilPolicyPreferencesSchema.parse(layer),
  );
  if (layers.length === 0)
    throw new VeilPolicyCompilationError(
      "VEIL_POLICY_EMPTY",
      "At least one policy layer is required",
    );

  const origins = layers
    .slice(1)
    .reduce(
  ...
  if (origins.length === 0)
    throw new VeilPolicyCompilationError(
      "VEIL_POLICY_NO_COMMON_ORIGIN",
      "Policy layers have no common allowed origin",
    );

  const controls = layers
    .map(resolveControls)
    .reduce((effective, layer) => intersectControls(effective, layer));
  ...
  const snapshot = veilPolicySnapshotSchema.parse({
    ...unsigned,
...
export function compileDefaultVeilPolicy(
  policy: Pick<ExecutionPolicy, "allowedOrigins" | "allowDownloads">,
): VeilPolicySnapshot {
...
export class VeilPolicyCompilationError extends Error {
  ...
  constructor(
    readonly code: "VEIL_POLICY_EMPTY" | "VEIL_POLICY_NO_COMMON_ORIGIN",
    message: string,
  ) {
    super(message);
  }
...
function resolveControls(
  layer: ReturnType<typeof veilPolicyPreferencesSchema.parse>,
): VeilPolicyControls {
...
function intersectControls(
  left: VeilPolicyControls,
  right: VeilPolicyControls,
): VeilPolicyControls {
  return Object.fromEntries(
    Object.keys(left).map((key) => [
      key,
      left[key as keyof VeilPolicyControls] && right[key as keyof VeilPolicyControls],
    ]),
  ) as VeilPolicyControls;
}
