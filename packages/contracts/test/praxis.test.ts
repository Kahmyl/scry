...
import {
  praxisDurableTransactionSchema,
  praxisFailureSchema,
  praxisLifecycleEventSchema,
  praxisRequestSchema,
  praxisResultSchema,
  praxisRunObservationSchema,
  praxisSuccessSchema,
} from "../src/praxis/index.js";
...
const intent = {
  ...
  risk: "ordinary",
  confidence: { requiredFamilies: [], minimum: 0.5, minimumMargin: 0.05, minimumFamilyCount: 2 },
} as const;
const request = {
  schemaVersion: 1,
  transactionId: "tx-1",
  ...
  privacy: { state: "normal", allowedChannels: ["dom", "accessibility"], suppressedChannels: [] },
...
const report = (
  ...
  classification: {
    provenance: outcome === "succeeded" ? "none" : "praxis",
    ...(outcome === "succeeded" ? {} : { code: "PRAXIS_FAILED" }),
    mutationOutcome,
  },
  intentDigest: "c".repeat(64),
  ...(outcome === "succeeded" ? { resolution } : {}),
  verification,
  timing,
  qualityFindings: [],
  safeActions: [],
...
describe("Praxis contracts", () => {
  it("parses the internal request and every terminal result shape", () => {
    ...
    const success = {
      ...
      timing,
      qualityFindings: [],
      report: report("succeeded", "applied"),
    };
    expect(praxisSuccessSchema.parse(success)).toEqual(success);
    expect(praxisResultSchema.parse(success)).toEqual(success);
    for (const status of ["failed", "inconclusive"] as const) {
      const failure = {
        ...
        timing,
        diagnostics: { reasonCode: "FAILED" },
        qualityFindings: [],
        safeActions: ["revise_intent"],
    ...
    const cancelled = {
      ...
      mutationOutcome: "not_started",
      timing,
      diagnostics: { reasonCode: "ABORT_SIGNAL" },
      qualityFindings: [],
      safeActions: ["retry_after_render"],
      report: {
        ...report("cancelled", "not_started"),
        classification: {
          provenance: "cancelled",
          code: "PRAXIS_CANCELLED",
          mutationOutcome: "not_started",
    ...
    expect(praxisFailureSchema.parse(cancelled)).toEqual(cancelled);
  });
  it("rejects incomplete results and unsafe retry combinations", () => {
    ...
    const unsafe = {
      ...
      qualityFindings: [],
      safeActions: [],
      report: report("inconclusive", "unknown"),
    };
    expect(() => praxisFailureSchema.parse(unsafe)).toThrow(/retry-safe/);
  });
  it("rejects protected values, low-level handles, and arbitrary coordinates", () => {
    expect(() =>
      praxisRequestSchema.parse({
        ...request,
        operation: {
          type: "enter_text",
          input: { reference: "secret-ref", classification: "known_secret" },
          value: "must-not-cross-contract",
        },
      }),
    ).toThrow();
    const success = {
      ...
      resolution: {
        ...resolution,
        target: { ...resolution.target, locator: "#save", x: 10, y: 20 },
      },
      verification,
      timing,
      qualityFindings: [],
    ...
    expect(() => praxisSuccessSchema.parse(success)).toThrow();
  });
  it("round-trips deterministically and validates lifecycle events", () => {
    ...
    expect(
      praxisLifecycleEventSchema.parse({
        schemaVersion: 1,
        transactionId: "tx-1",
        operationId: "save",
        type: "praxis.phase_changed",
        phase: "grounding",
  ...
  it("validates durable transaction and legacy-empty run projections", () => {
    ...
    expect(
      praxisRunObservationSchema.parse({
        contractVersion: 1,
        runtimeVersions: [],
        status: "complete",
        transactions: [],
    ...
    ).toMatchObject({ transactions: [], findings: [] });
  });
});
