import { z } from "zod";
import { praxisRunObservationSchema } from "./praxis/index.js";
import { veilRunObservationSchema } from "./veil/index.js";
...
export const recordingTimelineSchema = z
  .array(recordingTimelineEntrySchema)
  .superRefine((entries, context) => {
    for (const [index, entry] of entries.entries()) {
      if (entry.sequence !== index) {
        context.addIssue({
          code: "custom",
          message: "recording timeline sequence must be contiguous",
          path: [index, "sequence"],
        });
      }
      if (
        ...
        context.addIssue({
          code: "custom",
          message: "recording timeline entry ends before it starts",
          path: [index, "endedAt"],
        });
      }
    }
...
export const runObservationSectionStatusSchema = z.enum([
  "pending",
  "complete",
  "unavailable",
  "failed",
  ...
  .object({
    ...
    artifacts: z.array(
      z.object({
        ...
        privacyClassification: z.enum(["safe", "sanitized", "uncertain"]),
        failureProvenance: z.string().nullish(),
        reasonCode: z.string().nullish(),
        contentType: z.string(),
        sizeBytes: z.union([z.string(), z.number()]).nullish(),
        checksumSha256: z.string().nullish(),
        observation: z.record(z.string(), z.unknown()),
        resource: z
          .string()
          .regex(/^scry:\/\/artifact\/[0-9a-f-]+$/)
          .nullish(),
        createdAt: z.union([z.string(), z.date()]),
      }),
    ),
    ...
    privacy: z.object({
      ...
      credentialIncidents: z.array(z.record(z.string(), z.unknown())),
    }),
    veil: veilRunObservationSchema,
    praxis: praxisRunObservationSchema.default({
      contractVersion: 1,
      runtimeVersions: [],
      status: "complete",
      transactions: [],
    ...
    failure: z
      .object({
        provenance: z.enum(["product", "plan", "policy", "infrastructure", "privacy", "executor"]),
        code: z.string(),
      ...
      .nullish(),
    sections: z.object({
      attempts: runObservationSectionStatusSchema,
      steps: runObservationSectionStatusSchema,
      events: runObservationSectionStatusSchema,
      artifacts: runObservationSectionStatusSchema,
      timeline: runObservationSectionStatusSchema,
    }),
    integrity: z.object({
