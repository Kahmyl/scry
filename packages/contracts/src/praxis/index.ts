...
const expectedEffectTypeSchema = z.enum([
  "none",
  "navigation",
  "visibility_change",
  "value_change",
...
export const praxisFailureProvenanceSchema = z.enum([
  "application",
  "intent",
  "praxis",
  "policy",
...
export const praxisMutationOutcomeSchema = z.enum([
  "not_started",
  "not_applied",
  "applied",
  "unknown",
]);
export const praxisRetryDispositionSchema = z.enum([
  "safe",
  "unsafe",
...
export const praxisOperationSchema = z.discriminatedUnion("type", [
  ...
  z
    .object({
      type: z.literal("enter_text"),
      input: z
        .object({
          reference: identifier,
          classification: z.enum(["public", "known_secret", "captured_secret", "captured_public"]),
        })
        .strict(),
    })
    .strict(),
  z
    .object({
      ...
      input: z.object({ reference: identifier, classification: z.literal("public") }).strict(),
    })
    .strict(),
  ...
  z
    .object({
      type: z.literal("read_value"),
      classification: z.enum(["public", "known_secret", "unknown_secret"]),
      permittedMethods: z.array(identifier).min(1).max(10),
    })
    .strict(),
  z
    .object({
  ...
  z
    .object({
      ...
      direction: z.enum(["into_view", "up", "down", "left", "right"]),
    })
    .strict(),
]);
export const praxisTimingSchema = z
  .object({
    queuedMs: z.number().nonnegative().nullable(),
    observationMs: z.number().nonnegative().nullable(),
    groundingMs: z.number().nonnegative().nullable(),
    revalidationMs: z.number().nonnegative().nullable(),
    dispatchMs: z.number().nonnegative().nullable(),
    localVerificationMs: z.number().nonnegative().nullable(),
    effectVerificationMs: z.number().nonnegative().nullable(),
  ...
  .strict();
export const praxisQualityFindingSchema = z
  .object({
    code,
  ...
  .strict();
export const praxisTargetIdentitySchema = z
  .object({
    fingerprint: digest,
  ...
  .strict();
export const praxisResolutionSchema = z
  .object({
    target: praxisTargetIdentitySchema,
    confidence: z.number().min(0).max(1),
    runnerUpMargin: z.number().min(0).max(1),
    evidenceFamilies: z.array(evidenceFamilySchema).max(8),
    drift: driftClassificationSchema,
...
export const praxisVerificationSchema = z
  .object({
    ...
    effect: z.enum(["passed", "not_required", "failed", "unknown"]),
    effectType: expectedEffectTypeSchema,
  })
  .strict();
export const praxisAcquisitionOutputSchema = z
  .object({ classification: z.literal("public"), value: z.string().max(20_000) })
  .strict();
export const praxisAgentReportSchema = z
  .object({
    schemaVersion: z.literal(1),
    transactionId: identifier,
    operationId: identifier,
    stepId: identifier.optional(),
    outcome: z.enum(["succeeded", "failed", "inconclusive", "cancelled"]),
    summary: safeText,
    classification: z
      .object({
        provenance: z.union([praxisFailureProvenanceSchema, z.literal("none")]),
        code: code.optional(),
        mutationOutcome: praxisMutationOutcomeSchema,
      })
      .strict(),
    intentDigest: digest,
    resolution: praxisResolutionSchema.optional(),
    verification: praxisVerificationSchema,
    timing: praxisTimingSchema,
    qualityFindings: z.array(praxisQualityFindingSchema).max(100),
    safeActions: z.array(praxisSafeActionSchema).max(20),
    artifactRefs: z.array(z.string().regex(/^scry:\/\/artifact\/[0-9a-f-]+$/)).max(100),
  })
  .strict();
export const praxisRequestSchema = z
  .object({
    schemaVersion: z.literal(1),
    transactionId: identifier,
    operationId: identifier,
    ...
    operation: praxisOperationSchema,
    ...
    policy: z
      .object({
        ...
        actionTimeoutMs: z.number().int().min(100).max(60_000),
        totalTimeoutMs: z.number().int().min(100).max(120_000),
      })
      .strict(),
    privacy: z
      .object({
        state: identifier,
        allowedChannels: z.array(identifier).max(50),
        suppressedChannels: z.array(identifier).max(50),
      })
      .strict(),
    context: z
      .object({
...
const praxisSuccessBaseSchema = z
  .object({
    schemaVersion: z.literal(1),
    ...
    mutationOutcome: praxisMutationOutcomeSchema,
    resolution: praxisResolutionSchema,
    verification: praxisVerificationSchema,
    timing: praxisTimingSchema,
    output: praxisAcquisitionOutputSchema.optional(),
    qualityFindings: z.array(praxisQualityFindingSchema).max(100),
    report: praxisAgentReportSchema,
  })
  .strict();
export const praxisSuccessSchema = praxisSuccessBaseSchema.superRefine((value, context) => {
  if (value.report.outcome !== "succeeded")
    context.addIssue({
      code: "custom",
      message: "success report outcome must be succeeded",
      path: ["report", "outcome"],
    });
});
const praxisFailureBaseSchema = z
  .object({
    schemaVersion: z.literal(1),
    status: z.enum(["failed", "cancelled", "inconclusive"]),
    transactionId: identifier,
    operationId: identifier,
    stepId: identifier.optional(),
    phase: praxisPhaseSchema,
    code,
    provenance: praxisFailureProvenanceSchema,
    retry: praxisRetryDispositionSchema,
    mutationOutcome: praxisMutationOutcomeSchema,
    timing: praxisTimingSchema,
...
export const praxisFailureSchema = praxisFailureBaseSchema.superRefine((value, context) => {
  if (value.mutationOutcome === "unknown" && value.retry === "safe")
    context.addIssue({
      code: "custom",
      message: "unknown mutation outcome cannot be retry-safe",
      path: ["retry"],
    });
  if (value.status === "cancelled" && value.provenance !== "cancelled")
    context.addIssue({
      code: "custom",
      message: "cancelled result requires cancelled provenance",
      path: ["provenance"],
    });
  if (value.report.outcome !== value.status)
    context.addIssue({
      code: "custom",
      message: "report outcome must match result status",
      path: ["report", "outcome"],
    });
});
export const praxisResultSchema = z.union([praxisSuccessSchema, praxisFailureSchema]);
export const praxisLifecycleEventSchema = z
  .object({
    schemaVersion: z.literal(1),
    transactionId: identifier,
    operationId: identifier,
  ...
  .strict();
export const praxisDurableTransactionSchema = z
  .object({
    transactionId: identifier,
    operationId: identifier,
    stepId: identifier.nullish(),
    schemaVersion: z.literal(1),
    runtimeVersion: identifier,
    result: praxisResultSchema,
    startedAt: z.string().datetime(),
    completedAt: z.string().datetime().nullish(),
  })
  .strict();
export const praxisDurableQualityFindingSchema = z
  .object({
    id: identifier,
    transactionId: identifier,
    stepId: identifier.nullish(),
    intentDigest: digest,
    finding: praxisQualityFindingSchema,
    artifactRefs: z.array(z.string().regex(/^scry:\/\/artifact\/[0-9a-f-]+$/)).max(100),
    createdAt: z.string().datetime(),
  })
  .strict();
export const praxisRunObservationSchema = z
  .object({
    contractVersion: z.literal(1),
    runtimeVersions: z.array(identifier).max(20),
    status: z.enum(["pending", "complete", "unavailable", "failed"]),
    transactions: z.array(praxisDurableTransactionSchema).max(10_000),
    findings: z.array(praxisDurableQualityFindingSchema).max(10_000),
  })
  .strict();
