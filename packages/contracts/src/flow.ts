...
export const createFlowSchema = objectiveContextSchema.extend({
...
export const createFlowRevisionSchema = objectiveContextSchema.extend({
  environmentId: uuidSchema,
...
export const createFlowRunSchema = objectiveContextSchema.extend({
  environmentId: uuidSchema,
  ...
  }).strict().default({ width: 1280, height: 720 }),
  seed: z.number().int().min(0).max(4_294_967_295).default(1),
  role: z.enum(["exploratory", "diagnostic", "calibration", "candidate", "accepted", "superseded", "invalidated"]).default("candidate"),
}).strict();
