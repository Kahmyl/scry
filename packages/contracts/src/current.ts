...
import { assertionSchema, budgetsSchema, readinessSchema } from "./plan.js";
import {
  acquisitionIntentSchema,
  acquisitionReadinessContractSchema,
...
export const privacyFailureSchema = z
  .object({
...
export const safeResumeBoundarySchema = z.discriminatedUnion("kind", [
  z
    .object({
      kind: z.literal("known_secret_registered"),
      referenceType: z.enum(["vault", "captured"]),
    })
    .strict(),
  z
    .object({
      kind: z.literal("protected_surface_closed"),
      verifier: interactionTargetIntentSchema,
    })
    .strict(),
  z
    .object({
      ...
      urlPattern: z
        ...
        }, "navigation boundary pattern must be a valid regular expression"),
    })
    .strict(),
...
const transactionInputSchema = z.discriminatedUnion("classification", [
  z
    ...
    .strict(),
  z
    .object({ classification: z.literal("known_secret"), credentialRef: z.string().uuid() })
    .strict(),
]);
...
const preparationActionSchema = z.discriminatedUnion("type", [
  ...
  z
    .object({
      type: z.literal("clickNavigation"),
      target: interactionTargetIntentSchema,
      expectedEffect: expectedEffectSchema.default({ type: "none" }),
      effect: z.literal("read_only"),
      timeoutMs: timeout,
  ...
  z
    .object({
      type: z.literal("clickPublicInput"),
      input: identifier,
  ...
  z
    .object({
      type: z.literal("fillPublicInput"),
      input: identifier,
      target: interactionTargetIntentSchema,
      effect: z.literal("replayable_setup"),
      timeoutMs: timeout,
    })
    .strict(),
  z
    .object({
      type: z.literal("fillKnownSecret"),
      input: identifier,
      target: interactionTargetIntentSchema,
      effect: z.literal("replayable_setup"),
      timeoutMs: timeout,
  ...
  z
    .object({
      type: z.literal("selectPublicInput"),
      input: identifier,
      target: interactionTargetIntentSchema,
      effect: z.literal("replayable_setup"),
      timeoutMs: timeout,
  ...
  z
    .object({
      type: z.literal("checkPublicInput"),
      input: identifier,
      target: interactionTargetIntentSchema,
      effect: z.literal("replayable_setup"),
      timeoutMs: timeout,
...
const transactionAssertionSchema = z.union([
  ...
  z
    .object({
      type: z.literal("fieldValueMatchesInput"),
      target: interactionTargetIntentSchema,
      input: identifier,
      timeoutMs: timeout,
    })
  ...
  z
    .object({
      type: z.literal("textMatchesInput"),
      target: interactionTargetIntentSchema,
      input: identifier,
      exact: z.boolean().default(true),
      timeoutMs: timeout,
...
const transactionOutputSchema = z.discriminatedUnion("classification", [
  z
    .object({
      classification: z.literal("protected"),
  ...
  z
    .object({
      ...
      acquisition: acquisitionIntentSchema,
      storage: z
        .object({ name: z.string().trim().min(1).max(200), scope: z.enum(["run", "project"]) })
        .strict(),
    })
    .strict(),
]);

export const protectedTransactionSchema = z
  .object({
    type: z.literal("protectedTransaction"),
    operationId: identifier,
    entry: z
      .object({ url: z.string().url(), assertions: z.array(assertionSchema).min(1).max(20) })
      .strict(),
                  ...
                  .object({
                    origin: z
                      ...
                      .refine(
                        ...
                        "ignored request origin must not contain a path",
                      ),
    ...
    mutation: z
      .object({
        ...
        reconciliation: z.discriminatedUnion("strategy", [
          ...
          z
            .object({
              strategy: z.literal("persisted_outputs"),
              requiredReferences: z.array(identifier).min(1).max(8),
            })
            .strict(),
          z
            .object({
              strategy: z.literal("public_ui_state"),
              assertions: z.array(transactionAssertionSchema).min(1).max(20),
            })
            .strict(),
          z
            .object({
              strategy: z.literal("public_api_state"),
        ...
        strategies: z
          .array(
            z.discriminatedUnion("mode", [
              z
                .object({
                  mode: z.literal("resume_parked_context"),
                  reentryUrl: z.string().url(),
                  assertions: z.array(transactionAssertionSchema).min(1).max(20),
              ...
              z
                .object({
                  mode: z.literal("recreate_clean_context"),
              ...
              z
                .object({
                  mode: z.literal("reauthenticate"),
                  authenticationContractId: identifier,
                  reentryUrl: z.string().url(),
                  assertions: z.array(transactionAssertionSchema).min(1).max(20),
                  continueAtStepId: identifier,
                })
  ...
  .superRefine((transaction, context) => {
    const usedInputs = new Set(
      transaction.preparation.actions.flatMap((action) =>
        "input" in action ? [action.input] : [],
      ),
    );
    for (const input of Object.keys(transaction.inputs))
      if (!usedInputs.has(input))
        context.addIssue({
          code: "custom",
          message: `unused transaction input: ${input}`,
          path: ["inputs", input],
        });
    for (const [index, action] of transaction.preparation.actions.entries()) {
      ...
      if (!input)
        context.addIssue({
          code: "custom",
          message: `undeclared transaction input: ${action.input}`,
          path: ["preparation", "actions", index, "input"],
        });
      if (action.type === "fillKnownSecret" && input?.classification !== "known_secret")
        context.addIssue({
          code: "custom",
          message: "fillKnownSecret requires a known_secret input",
          path: ["preparation", "actions", index, "input"],
        });
      if (action.type !== "fillKnownSecret" && input?.classification === "known_secret")
        context.addIssue({
          code: "custom",
          message: "known_secret inputs may only be used by fillKnownSecret",
          path: ["preparation", "actions", index, "input"],
        });
      if (action.type === "clickPublicInput" && input?.classification !== "public")
        context.addIssue({
          code: "custom",
          message: "clickPublicInput requires a public input",
          path: ["preparation", "actions", index, "input"],
        });
    }
    const verifiedInputs = new Set(
      transaction.preparation.assertions.flatMap((assertion) =>
        assertion.type === "fieldValueMatchesInput" || assertion.type === "textMatchesInput"
          ? [assertion.input]
          : [],
      ),
    );
    const inputsRequiringValueAssertion = new Set(
      transaction.preparation.actions.flatMap((action) =>
        action.type === "fillPublicInput" ||
        action.type === "fillKnownSecret" ||
        action.type === "selectPublicInput" ||
        action.type === "checkPublicInput"
          ? [action.input]
          : [],
      ),
    );
    for (const input of inputsRequiringValueAssertion) {
      if (!verifiedInputs.has(input))
        context.addIssue({
          code: "custom",
          message: `transaction input lacks a post-preparation value assertion: ${input}`,
          path: ["preparation", "assertions"],
        });
    }
    if (
      !transaction.preparation.assertions.some(
        (assertion) => assertion.type === "enabled" || assertion.type === "visible",
      )
    ) {
      context.addIssue({
        code: "custom",
        message: "preparation requires a mutation-readiness assertion",
        path: ["preparation", "assertions"],
      });
    }
    const transactionAssertions = [
      ...transaction.preparation.assertions,
      ...(transaction.mutation.reconciliation.strategy === "public_ui_state"
        ? transaction.mutation.reconciliation.assertions
        : []),
      ...transaction.continuation.strategies.flatMap((strategy) =>
        "assertions" in strategy ? strategy.assertions : [],
      ),
    ];
    for (const assertion of transactionAssertions) {
      ...
      if (!input)
        context.addIssue({
          code: "custom",
          message: `assertion references undeclared transaction input: ${assertion.input}`,
          path: ["preparation", "assertions"],
        });
      if (assertion.type === "textMatchesInput" && input?.classification !== "public")
        context.addIssue({
          code: "custom",
          message: "textMatchesInput may only expose public transaction inputs",
          path: ["preparation", "assertions"],
        });
    }
    const outputRefs = new Set<string>();
    for (const [outputIndex, output] of transaction.extraction.outputs.entries()) {
      if (outputRefs.has(output.reference))
        context.addIssue({
          code: "custom",
          message: `duplicate transaction output: ${output.reference}`,
          path: ["extraction", "outputs", outputIndex, "reference"],
        });
      outputRefs.add(output.reference);
      if (
        (output.classification === "protected" &&
          output.acquisition.classification !== "unknown_secret") ||
        (output.classification === "public" && output.acquisition.classification !== "public")
      )
        context.addIssue({
          code: "custom",
          message: "output and acquisition classifications must match",
          path: ["extraction", "outputs", outputIndex, "acquisition", "classification"],
        });
    }
    if (transaction.mutation.reconciliation.strategy === "persisted_outputs") {
      for (const [
        index,
        reference,
      ] of transaction.mutation.reconciliation.requiredReferences.entries()) {
        if (!outputRefs.has(reference))
          context.addIssue({
            code: "custom",
            message: `persisted-output reconciliation references an undeclared output: ${reference}`,
            path: ["mutation", "reconciliation", "requiredReferences", index],
          });
      }
    }
    if (
      transaction.mutation.kind === "one_time" &&
      transaction.mutation.reconciliation.strategy === "none" &&
      !transaction.mutation.reconciliation.acceptUnknownOutcome
    ) {
      context.addIssue({
        code: "custom",
        message: "one-time mutation requires reconciliation or explicit unknown-outcome acceptance",
        path: ["mutation", "reconciliation"],
      });
    }
  });

export const capturePublicValueSchema = z
  .object({
  ...
  .superRefine((operation, context) => {
    if (operation.capture.acquisition.classification !== "public")
      context.addIssue({
        code: "custom",
        message: "public-value capture requires public acquisition",
        path: ["capture", "acquisition", "classification"],
      });
  });

export const currentActionSchema = z.union([
  z
    .object({ type: z.literal("navigate"), url: z.string().min(1).max(2_048), timeoutMs: timeout })
    .strict(),
  z
    .object({
      type: z.literal("click"),
    ...
    .refine(
      (value) =>
        [
          value.value,
          value.secretRef,
          value.capturedSecretRef,
          value.capturedValueRef,
          value.generatedValueRef,
...
export const currentStepSchema = z
  .object({
    id: identifier,
    ...
    evidence: z
      .array(z.enum(["screenshot", "dom", "network"]))
      .max(3)
      .default([]),
  ...
  .object({
    ...
    state: z
      .object({
        ...
        localStorage: z.literal(true),
        indexedDb: z.boolean().default(true),
      })
      .strict(),
  })
  .strict();
...
export const currentPlanSchema = z
  ...
  .superRefine((plan, context) => {
    ...
    if (plan.steps.length > plan.budgets.maxActions) {
      context.addIssue({
        code: "custom",
        message: "step count exceeds the action budget",
    ...
    for (const [index, step] of plan.steps.entries()) {
      ...
      if (step.action.type === "protectedTransaction") {
        for (const output of step.action.extraction.outputs) {
          if (references.has(output.reference))
            context.addIssue({
              code: "custom",
              message: `duplicate captured reference: ${output.reference}`,
              path: ["steps", index, "action", "extraction", "outputs"],
            });
          references.add(output.reference);
        }
      } else if (step.action.type === "capturePublicValue") {
        if (references.has(step.action.reference))
          context.addIssue({
            code: "custom",
            message: `duplicate captured reference: ${step.action.reference}`,
            path: ["steps", index, "action", "reference"],
          });
      ...
      if (step.action.type === "protectedTransaction" && step.onFailure === "continue") {
        context.addIssue({
          code: "custom",
          message: "protected transactions use their continuation contract, not onFailure continue",
          path: ["steps", index, "onFailure"],
        });
      }
      if (step.action.type === "protectedTransaction") {
        if (!plan.allowedOrigins.includes(new URL(step.action.entry.url).origin))
          context.addIssue({
            code: "custom",
            message: "transaction entry origin must be allowed",
            path: ["steps", index, "action", "entry", "url"],
          });
        for (const [outputIndex, output] of step.action.extraction.outputs.entries()) {
          if (
            output.acquisition.permittedMethods.includes("protected_visual_reading") &&
            !output.acquisition.target.preferredEvidence.visual?.protectedUse
          )
            context.addIssue({
              code: "custom",
              message:
                "protected visual acquisition requires an explicitly protected visual target",
              path: [
                "steps",
                index,
                "action",
        ...
        const terminalIndex = strategies.findIndex(
          ({ mode }) => mode === "terminal" || mode === "continue_unrecorded",
        );
        if (terminalIndex >= 0 && terminalIndex !== strategies.length - 1) {
          context.addIssue({
            code: "custom",
            message: "terminal and unrecorded continuation strategies must be last",
            path: ["steps", index, "action", "continuation", "strategies", terminalIndex],
          });
        }
        for (const [strategyIndex, strategy] of strategies.entries()) {
          ...
          if (!plan.allowedOrigins.includes(new URL(strategy.reentryUrl).origin)) {
            context.addIssue({
              code: "custom",
              message: "continuation re-entry origin must be allowed",
              path: [
                "steps",
                index,
                "action",
          ...
          if (continuationIndex === undefined || continuationIndex <= index) {
            context.addIssue({
              code: "custom",
              message: "continuation must target a later step",
              path: [
                "steps",
                index,
                "action",
          ...
          if (strategy.mode === "recreate_clean_context") {
            ...
            if (!checkpoint || checkpoint.beforeStepId !== step.id)
              context.addIssue({
                code: "custom",
                message: "clean recreation requires a checkpoint immediately before this operation",
                path: [
                  "steps",
                  index,
                  "action",
        ...
        if (strategies.some(({ mode }) => mode === "continue_unrecorded")) {
          for (let laterIndex = index + 1; laterIndex < plan.steps.length; laterIndex += 1) {
            ...
            if (later.action.type === "screenshot" || later.evidence.length > 0)
              context.addIssue({
                code: "custom",
                message: "steps reachable by unrecorded continuation cannot require evidence",
                path: ["steps", laterIndex, "evidence"],
              });
          }
        }
      }
    ...
    if (navigationCount > plan.budgets.maxNavigations) {
      context.addIssue({
        code: "custom",
        message: "navigation count exceeds the navigation budget",
    ...
    for (const [checkpointIndex, checkpoint] of plan.checkpoints.entries()) {
      ...
      if (before === undefined || plan.steps[before]?.action.type !== "protectedTransaction")
        context.addIssue({
          code: "custom",
          message: "checkpoint beforeStepId must identify a protected transaction",
          path: ["checkpoints", checkpointIndex, "beforeStepId"],
        });
      if (!plan.allowedOrigins.includes(new URL(checkpoint.restorationUrl).origin))
        context.addIssue({
          code: "custom",
          message: "checkpoint restoration origin must be allowed",
          path: ["checkpoints", checkpointIndex, "restorationUrl"],
        });
    }
  });
...
function validateCapabilityRequirements(
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) => validateCapabilityRequirements(item, [...path, index], context));
    return;
  }
  ...
  const requireAll = (capabilities: string[]) => {
    for (const capability of capabilities)
      if (!required.includes(capability))
        context.addIssue({
          code: "custom",
          message: `${String(item.type)} requires target capability ${capability}`,
          path: [...path, "target", "requiredCapabilities"],
        });
  };
  if (["fill", "fillPublicInput", "fillKnownSecret"].includes(String(item.type)))
    requireAll(["focusable", "accepts_text", "editable"]);
  ...
  if (["fieldValueMatchesInput", "value"].includes(String(item.type)))
    requireAll(["readable_value"]);
  if (String(item.type) === "checked") requireAll(["toggleable"]);
  if (
    ["click", "clickNavigation"].includes(String(item.type)) &&
    target &&
    !required.includes("pointer_activatable") &&
    !required.includes("keyboard_activatable")
  )
    context.addIssue({
      code: "custom",
      message: `${String(item.type)} requires pointer_activatable or keyboard_activatable`,
      path: [...path, "target", "requiredCapabilities"],
    });
  for (const [key, child] of Object.entries(item))
    validateCapabilityRequirements(child, [...path, key], context);
}

const channelResultSchema = z
  .object({
...
const safeActionSchema = z.enum([
  ...
  "retry_acquisition",
  "request_secure_assistance",
  "revoke_credential",
  "abandon_credential",
]);
export const extractionDiagnosticSchema = z
  .object({
    candidate: z.number().int().min(0),
    attempts: z.number().int().min(0),
    firstAttemptedAt: z.string().datetime().optional(),
    durationMs: z.number().int().min(0),
    containerResolved: z.boolean(),
    matchCount: z.enum(["none", "one", "many", "unknown"]),
    visibility: z.enum(["visible", "hidden", "unknown"]),
    accessibility: z.enum(["available", "unavailable", "unknown"]),
    lastFailureCode: identifier.optional(),
  })
  .strict();
const phaseResultSchema = z
  .object({
    status: z.enum(["not_started", "running", "succeeded", "failed", "incomplete"]),
    code: identifier.optional(),
  })
  .strict();
export const protectedTransactionResultSchema = z
  .object({
    status: z.enum([
      "completed",
      "aborted",
    ...
    mutation: z
      .object({
        dispatch: z.enum(["not_started", "authorized", "started", "acknowledged"]),
        outcome: z.enum([
          "not_attempted",
          "confirmed_succeeded",
    ...
    failurePhase: z
      ...
      .optional(),
    retryClass: z
      .enum(["safe_to_retry", "retry_requires_reconciliation", "do_not_retry", "manual_review"])
      .optional(),
    safeActions: z.array(safeActionSchema),
    preparationEffects: z
      .array(
        z
          .object({
