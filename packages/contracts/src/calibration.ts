...
export const requestCalibrationSchema = z
  .object({
    missionId: uuid,
    objectiveId: uuid,
    agentSessionId: uuid,
...
export const decideCalibrationSchema = objectiveContextSchema
  .extend({
...
export const bindCalibrationSchema = z
  .object({
    missionId: uuid,
    objectiveId: uuid,
    agentSessionId: uuid,
...
export const retryCalibrationSchema = objectiveContextSchema
  .extend({
    idempotencyKey: z.string().trim().min(8).max(200),
  })
  .strict();
