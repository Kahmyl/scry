...
import { VeilAuthority, VeilAuthorityError } from "@scry/veil";
...
export type PraxisVeilBoundary =
  | "schedule"
  | "observe"
  | "interact"
...
export class PraxisVeilAuthorizationError extends Error {
  ...
  constructor(
    readonly boundary: PraxisVeilBoundary,
    cause: unknown,
  ) {
...
export function registerPraxisVeilAuthority(page: Page, registration: Registration): () => void {
  ...
  return () => {
    if (registrations.get(page) === registration) registrations.delete(page);
  };
}
...
export function authorizePraxisRequest(page: Page, request: PraxisRequest): PraxisRequest {
  ...
  if (!registration)
    throw new PraxisVeilAuthorizationError("schedule", new Error("VEIL_PRAXIS_AUTHORITY_REQUIRED"));
  const allowed: string[] = [];
  const suppressed = new Set(request.privacy.suppressedChannels);
  const authorized = {
    ...request,
    privacy: { ...request.privacy, allowedChannels: allowed, suppressedChannels: [...suppressed] },
  };
  ...
  for (const praxisChannel of request.privacy.allowedChannels) {
    ...
    try {
      requireGrant(
        ...
        "observe",
        channel,
        praxisChannel === "protected" ? "sensitive" : "public",
        "channel",
      );
...
export function validatePraxisVeilBoundary(
  request: PraxisRequest,
  boundary: PraxisVeilBoundary,
): void {
  if (boundary === "observe") {
  ...
  try {
    state.registration.authority.validateLease(grant.lease, grant.request);
  } catch (error) {
    throw new PraxisVeilAuthorizationError(boundary, error);
  }
}
...
function validateObservationGrants(request: PraxisRequest): void {
  ...
  for (const [key, grant] of state.grants) {
    ...
    try {
      ...
      throw new PraxisVeilAuthorizationError("observe", error);
    }
  }
}
...
function requireGrant(
  ...
  classification: VeilLeaseRequest["classification"],
  scope: VeilLeaseRequest["scope"],
  boundary: PraxisVeilBoundary = operation === "observe" ? "observe" : "schedule",
): Grant {
  const state = authorization(request, boundary);
  ...
  const leaseRequest: VeilLeaseRequest = {
    context: {
      ...
      environmentId: state.registration.environmentId,
      transactionId: request.transactionId,
      origin: request.context.origin,
      browserContextId: state.registration.browserContextId,
      pageId: request.context.pageId,
      frameId: "main-frame",
      documentEpoch: request.context.documentEpoch,
    },
    operation,
    channel,
    classification,
...
function authorization(request: PraxisRequest, boundary: PraxisVeilBoundary): Authorization {
  ...
  if (!state || state.released)
    throw new PraxisVeilAuthorizationError(boundary, new Error("VEIL_PRAXIS_GRANT_REQUIRED"));
  if (state.binding && state.binding !== digestRequest(request))
    throw new PraxisVeilAuthorizationError(boundary, new Error("VEIL_PRAXIS_REQUEST_CHANGED"));
  return state;
}
...
function digestRequest(request: PraxisRequest): string {
