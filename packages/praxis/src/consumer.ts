...
import { PraxisAdapter, type PraxisInputResolver } from "./adapter.js";
...
export async function executePraxisConsumer(input: PraxisConsumerInput): Promise<PraxisResult> {
  ...
  try {
    const result = await new PraxisTransactionCoordinator(
      new PraxisAdapter(input.page, input.resolveInput),
      input.context.emit,
...
export class PraxisConsumerError extends Error {
  constructor(readonly result: Exclude<PraxisResult, { status: "succeeded" }>) {
    super(result.code);
    this.name = "PraxisConsumerError";
  }
}
...
async function buildPraxisRequest(input: PraxisConsumerInput): Promise<PraxisRequest> {
  ...
  const seed = [
    input.context.runId ?? "standalone",
    input.context.attemptId ?? "attempt",
    input.context.stepId ?? "step",
    input.context.channel,
    input.context.ordinal,
  ...
  return {
    ...
    policy: {
      ...
      actionTimeoutMs: bounded(input.context.timeoutMs, 100, 60_000),
      totalTimeoutMs: bounded(input.context.timeoutMs, 100, 120_000),
    },
    privacy: input.context.privacy ?? {
      state: "normal",
      allowedChannels: ["public_dom", "accessibility", "visual"],
      suppressedChannels: [],
    },
    context: {
      ...(input.context.runId ? { runId: input.context.runId } : {}),
      ...(input.context.attemptId ? { attemptId: input.context.attemptId } : {}),
      pageId: `page-${createHash("sha256").update(origin).digest("hex").slice(0, 16)}`,
      origin,
      documentEpoch: epoch,
    },
  };
...
function safeOrigin(url: string, fallback?: string) {
  try {
    ...
    /* use the declared allowed origin */
  }
