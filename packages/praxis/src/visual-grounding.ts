...
const eng = createRequire(import.meta.url)("@tesseract.js-data/eng") as {
  code: string;
  gzip: boolean;
  langPath: string;
};
...
async function worker() {
  workerPromise ??= createWorker(eng.code, OEM.LSTM_ONLY, {
...
export async function resolveVisualAnchors(
  page: Page,
  scope: Locator,
  ...
  if (
    (visual.sources.includes("ocr") || visual.sources.includes("canvas")) &&
    visual.expectedText
  ) {
    ...
    try {
      ...
      for (const word of flattenWords(result.data.blocks ?? [])) {
        ...
        anchors.push({
          text: word.text,
          bounds: {
            x: scopeBox.x + word.bbox.x0,
            y: scopeBox.y + word.bbox.y0,
            width: word.bbox.x1 - word.bbox.x0,
            height: word.bbox.y1 - word.bbox.y0,
          },
          confidence: Math.min(1, (similarity * Math.max(0, word.confidence)) / 100),
          source: visual.sources.includes("canvas") ? "canvas" : "ocr",
        });
      }
    } finally {
      image.fill(0);
  ...
  if (visual.sources.includes("icon") && visual.icon) {
    const aliases = iconAliases(visual.icon);
    const icons = scope.locator("button, a, [role=button], svg, img, canvas");
    for (let index = 0, count = Math.min(await icons.count(), 500); index < count; index += 1) {
      ...
      if (!meta || !aliases.some((alias) => meta.identity.split(/[^a-z]+/).includes(alias)))
        continue;
      anchors.push({
        text: visual.icon,
        bounds: meta.bounds,
        confidence: 0.92,
        source: meta.canvas ? "canvas" : "icon",
      });
    }
  }
  if (visual.sources.includes("geometry")) {
    const expected =
      ...
      intent.preferredEvidence.names[0] ??
      intent.preferredEvidence.labels[0];
    if (expected) {
      const texts = scope.getByText(expected, { exact: false });
      for (let index = 0, count = Math.min(await texts.count(), 20); index < count; index += 1) {
        const box = await texts.nth(index).boundingBox();
        if (box) anchors.push({ text: expected, bounds: box, confidence: 0.82, source: "ocr" });
      }
    }
  }
  return anchors.sort(
    (a, b) => b.confidence - a.confidence || a.bounds.y - b.bounds.y || a.bounds.x - b.bounds.x,
  );
}

export async function acquireProtectedVisualText(
  page: Page,
  intent: InteractionTargetIntent,
): Promise<string> {
  ...
  const minimum = intent.confidence.minimum ?? 0.82,
    margin = (anchors[0]?.confidence ?? 0) - (anchors[1]?.confidence ?? 0);
  if (
    !anchors[0]?.text ||
    (anchors[0]?.confidence ?? 0) < minimum ||
    margin < (intent.confidence.minimumMargin ?? 0.15)
  )
    throw new Error(anchors.length > 1 ? "TARGET_AMBIGUOUS" : "INSUFFICIENT_EVIDENCE");
...
function iconAliases(
  icon: NonNullable<NonNullable<InteractionTargetIntent["preferredEvidence"]["visual"]>["icon"]>,
): string[] {
  return (
    {
      add: ["add", "plus", "new"],
      back: ["back", "left", "previous"],
      check: ["check", "done", "confirm"],
      close: ["close", "dismiss", "x"],
      copy: ["copy", "duplicate"],
      delete: ["delete", "trash", "remove"],
      download: ["download"],
      edit: ["edit", "pencil"],
      forward: ["forward", "right", "next"],
      menu: ["menu", "hamburger"],
      more: ["more", "ellipsis"],
