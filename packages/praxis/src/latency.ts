export type PraxisPhaseBudget =
  ...
  | "localVerification"
  | "effectVerification";
const shares: Record<PraxisPhaseBudget, number> = {
  observation: 0.18,
  grounding: 0.32,
  revalidation: 0.08,
  dispatch: 0.16,
  localVerification: 0.1,
  effectVerification: 0.16,
};
...
export class PraxisBudget {
  ...
  remaining() {
    return Math.max(0, this.totalMs - (performance.now() - this.startedAt));
  }
  forPhase(phase: PraxisPhaseBudget) {
    return Math.max(0, Math.min(this.remaining(), this.totalMs * shares[phase]));
  }
  exhausted() {
...
export function escalationLevel(input: {
  ...
  semanticAuthoritative: boolean;
  behaviorRequired: boolean;
  visualUsed: boolean;
  adapterUsed: boolean;
  budgetExhausted: boolean;
}): PraxisEscalationLevel {
...
export class PraxisMetrics {
  ...
  timing(phase: string, durationMs: number, dimensions: Record<string, string> = {}) {
    this.sink({
      name: "praxis.phase.duration_ms",
      value: durationMs,
      dimensions: { phase, ...dimensions },
    });
  }
  escalation(level: PraxisEscalationLevel, dimensions: Record<string, string> = {}) {
    this.sink({ name: "praxis.escalation", value: level, dimensions });
  }
}
