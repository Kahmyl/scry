...
const evidence: CandidateEvidence[] = [
  { family: "textual", signal: "name", score: 0.9, correlationGroup: "identity" },
  { family: "accessibility", signal: "name", score: 0.8, correlationGroup: "accessible" },
  { family: "textual", signal: "duplicate", score: 0.4, correlationGroup: "identity" },
];

describe("Praxis observation and grounding foundation", () => {
  it("scores deterministically regardless of provider completion order", () => {
    expect(PraxisGroundingPolicyV1.score(evidence)).toEqual(
      PraxisGroundingPolicyV1.score([...evidence].reverse()),
    );
  });

  it("does not run a provider whose privacy channel is forbidden", async () => {
    let observed = false;
    const provider: PraxisEvidenceProvider<object> = {
      id: "ocr",
      version: 1,
      families: ["visual"],
      cost: "high",
    ...
    const [result] = await runEvidenceProviders(
      [provider],
      {},
      ["public_dom"],
  ...
  it("serializes only opaque handle identity and rejects a stale handle", async () => {
    ...
    await expect(handle.use((locator) => locator.click())).rejects.toThrow(
  ...
  it("invalidates cached observations when the document epoch changes", async () => {
    ...
    const epoch = await PraxisDocumentEpoch.current(page);
    const key = PraxisObservationCache.key({
      scope: { kind: "page" },
      privacyState: "normal",
      providers: ["native"],
      epoch,
    ...
    const snapshot = {
      ...observationIdentity(page, { kind: "page" }, "normal", epoch),
      controls: [],
      providerTimings: [],
    } as const;
