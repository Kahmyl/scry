...
const unsafeChannels: EvidenceChannel[] = [
  "trace",
  "screenshot",
  "dom",
  "accessibility",
...
describe("Scry Privacy Gauntlet", () => {
  it("admits no evidence-bearing channel during a recording-gap operation", async () => {
    ...
    for (const channel of unsafeChannels)
      expect(gate.getDecision(channel), channel).toBe("suppress");
    for (const channel of sanitizedChannels)
      expect(gate.getDecision(channel), channel).toBe("sanitize");
  });
