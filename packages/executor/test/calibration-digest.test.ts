...
const intent = (
  concept: string,
  role: "button" | "textbox" | "value" = "textbox",
  ...
  risk: "credential",
  confidence: { requiredFamilies: [], minimumFamilyCount: 3 },
});
...
function transaction(name = "Disposable app"): ProtectedTransaction {
  return {
    ...
    entry: {
      ...
      assertions: [{ type: "visible", target: intent("Name") }],
    },
    ...
    mutation: {
      action: {
        ...
        expectedEffect: { type: "new_region", target: intent("Secret", "value") },
      },
      kind: "one_time",
    ...
    acquisitionReadiness: {
      ceremonyIntent: intent("Issue", "button"),
      ...
      valueIntent: intent("Secret", "value"),
...
describe("protected transaction digests", () => {
  it("separates program, input schema, and concrete input identity", () => {
    const baseline = transaction("Calibration app");
    const production = transaction("Production app");
    expect(protectedTransactionDigest(production, ["https://example.test/path"])).toBe(
      protectedTransactionDigest(baseline, ["https://example.test"]),
    );
    expect(transactionInputSchemaDigest(production)).toBe(transactionInputSchemaDigest(baseline));
    expect(transactionInputDigest(production, "key")).not.toBe(
      transactionInputDigest(baseline, "key"),
    );
  });

  it("changes for execution-sensitive boundaries but ignores binding and display names", () => {
    const baseline = transaction();
    const digest = protectedTransactionDigest(baseline, ["https://example.test"]);
    const rebound = {
      ...baseline,
      calibrationAttestationId: "11111111-1111-4111-8111-111111111111",
      extraction: {
        ...baseline.extraction,
        outputs: baseline.extraction.outputs.map((output) =>
          output.classification === "protected"
            ? { ...output, storage: { ...output.storage, credentialName: "Renamed" } }
            : output,
        ),
      },
    };
    ...
    expect(
      protectedTransactionDigest(
        { ...baseline, mutation: { ...baseline.mutation, kind: "repeatable" } },
        ["https://example.test"],
      ),
    ).not.toBe(digest);
    expect(
      protectedTransactionDigest(
        { ...baseline, preparation: { ...baseline.preparation, actions: [] as never } },
        ["https://example.test"],
      ),
    ).not.toBe(digest);
  });
