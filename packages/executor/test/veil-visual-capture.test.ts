...
import { visualRedactionInitScript } from "../src/browser-runtime-artifacts.js";
import { VeilVisualCaptureAuthority, VeilVisualCaptureError } from "../src/veil-visual-capture.js";

const policyDigest = "a".repeat(64);
const binding = { browserContextId: "context-1", pageId: "page-1", frameId: "main-frame", documentEpoch: 4 } as const;
let browser: Browser;
let page: Page;
...
describe("VeilVisualCaptureAuthority", () => {
  it("discovers and masks sensitive and opaque surfaces before issuing a bound permit", async () => {
    ...
    expect(await page.locator("input").evaluate((element) => getComputedStyle(element).filter)).not.toBe("none");
  ...
  it("refuses stale geometry and changed documents", async () => {
    ...
    expect(await page.locator("#dynamic-secret").evaluate((element) => getComputedStyle(element).filter)).not.toBe("none");
    await expect(authority.validate(page, permit, binding)).rejects.toMatchObject({ code: "VEIL_CAPTURE_DOCUMENT_CHANGED" });
  });
  ...
  it("refuses cross-context, stale-epoch, expired, forged, and replayed permits", async () => {
    ...
    await expect(authority.validate(page, first.permit, { ...binding, documentEpoch: 5 })).rejects.toMatchObject({ code: "VEIL_CAPTURE_CONTEXT_MISMATCH" });
    await expect(authority.validate(page, { ...first.permit, maskDigest: "b".repeat(64) }, binding)).rejects.toMatchObject({ code: "VEIL_CAPTURE_PERMIT_INVALID" });
    now = 1_101;
    await expect(authority.validate(page, first.permit, binding)).rejects.toMatchObject({ code: "VEIL_CAPTURE_PERMIT_EXPIRED" });

    now = 2_000;
    const second = await authority.issue(page, binding);
    await authority.capture(page, second.permit, binding, async () => "captured");
    await expect(authority.validate(page, second.permit, binding)).rejects.toBeInstanceOf(VeilVisualCaptureError);
  });
});
