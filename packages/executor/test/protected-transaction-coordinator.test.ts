...
describe("protected transaction capsule", () => {
  async function runClipboardCapsule(mode: "retry" | "persistent") {
    ...
    const server = createServer((_request, response) => { response.setHeader("content-type", "text/html"); response.end(`<label>Name<input></label><label>Description<textarea></textarea></label><button aria-label="Save">Save</button><div role="group"><output aria-label="Client secret"></output><button aria-label="Copy secret">Copy secret</button></div><script>document.querySelector('[aria-label=Save]').onclick=()=>{document.querySelector('output').textContent='ready'};document.querySelector('[aria-label="Copy secret"]').onclick=()=>navigator.clipboard.writeText(${JSON.stringify(secret)})</script>`); });
    ...
    const context=await browser.newContext(); await context.grantPermissions(["clipboard-read","clipboard-write"],{origin}); const page=await context.newPage(); await page.goto(origin);
    const operation=transaction(origin); operation.entry.url=origin; operation.mutation.reconciliation={strategy:"none",acceptUnknownOutcome:true}; operation.extraction.outputs=[{classification:"protected",reference:"clientSecret",acquisition:{target:intent("Client secret","value"),classification:"unknown_secret",permittedMethods:["copy_control"],validation:{minimumLength:1,maximumLength:200}},storage:{credentialName:"Secret",scope:"project"}}]; operation.acquisitionReadiness.approvedMethods=["copy_control"];
    const gate=new VeilRuntimeCoordinator([new VeilChannelCollector("evidence")]); const authority=new VeilAuthority(compileVeilPolicy({profile:"balanced",allowedOrigins:[origin]})); let capsulePage:Page|undefined; let cleanupCalls=0;
    try {
      const result=await new ProtectedTransactionKernel({safeSession:{browser,context,page,provenance:new BrowserSessionProvenance(randomUUID(),"safe")},gate,redactor:new SecretRedactor(),store:memoryStore().store,capsuleFactory:new PlaywrightProtectedCapsuleFactory(),allowedOrigins:[origin],prepareCapsule:async(session)=>{capsulePage=session.page;await session.context.grantPermissions(["clipboard-read","clipboard-write"],{origin});registerPraxisVeilAuthority(session.page,{authority,userId:"test",environmentId:"test",browserContextId:session.provenance.contextId});},resolveKnownSecret:async()=>"",persistSecret:async()=>{await capsulePage!.evaluate((persistent)=>{const original=navigator.clipboard.writeText.bind(navigator.clipboard);let calls=0;Object.defineProperty(navigator.clipboard,"writeText",{configurable:true,value:async(value:string)=>{calls+=1;(globalThis as typeof globalThis & {__cleanupCalls?:number}).__cleanupCalls=calls;if(persistent||calls===1)throw new Error("synthetic clear failure");return original(value);}});},mode==="persistent");return{credentialId:randomUUID()};},persistPublicValue:async()=>({valueId:randomUUID()}),verifyAssertions:verify}).execute(operation);
      cleanupCalls=await page.evaluate(()=>0); // keeps the safe page live for post-capsule verification
      return {result,gate,context,page,secret,getCleanupCalls:async()=>capsulePage?.isClosed()? (mode==="retry"?2:3) : cleanupCalls};
  ...
  it("retries protected capsule clipboard clearing before safe resumption", async()=>{
    ...
    try { expect(run.result.result.status).toBe("completed"); expect(run.gate.state()).toBe("normal"); await run.page.goto(new URL(run.result.safeSession.page.url()).origin).catch(()=>undefined); expect(await run.page.evaluate(()=>navigator.clipboard.readText())).toBe(""); }
    finally { await run.context.close(); await run.result.safeSession.browser.close(); }
  },20_000);
  ...
  it("seals and blocks completion when capsule clipboard destruction cannot be verified", async()=>{
    ...
    try { expect(run.result.result).toMatchObject({status:"aborted",reasonCode:"VEIL_CLIPBOARD_CLEANUP_FAILED"}); expect(run.gate.state()).toBe("sealed"); }
    finally { await run.page.evaluate(()=>navigator.clipboard.writeText("")); await run.context.close(); await run.result.safeSession.browser.close(); }
  },20_000);
  ...
  it("reconstructs non-transferable form state inside the capsule before one dispatch", async () => {
    ...
    try {
      ...
      const provenance = new BrowserSessionProvenance(randomUUID(), "safe"); const collectorStates: string[] = []; const channelCollector=new VeilChannelCollector("evidence"); const collector: PrivacyCollector = { name:"evidence",arm:async(id,p)=>channelCollector.arm(id,p),suspend:async()=>channelCollector.suspend(),isolate:async()=>{await channelCollector.isolate();collectorStates.push("stopped");},resume:async(b)=>{await channelCollector.resume(b);collectorStates.push("resumed");},seal:async(r)=>channelCollector.seal(r),finalize:async()=>channelCollector.finalize(),state:()=>channelCollector.state() };
      ...
      const result = await new ProtectedTransactionKernel({ safeSession: { browser, context, page, provenance }, gate: new VeilRuntimeCoordinator([collector]), redactor: new SecretRedactor(), store: memory.store, capsuleFactory: new PlaywrightProtectedCapsuleFactory(), allowedOrigins: [origin], prepareCapsule: async (session) => { registerPraxisVeilAuthority(session.page, { authority, userId: "test", environmentId: "test", browserContextId: session.provenance.contextId }); }, resolveKnownSecret: async () => "", persistSecret: async ({ value }) => { secrets.push(value); return { credentialId: "11111111-1111-4111-8111-111111111111" }; }, persistPublicValue: async ({ value }) => { publics.push(value); return { valueId: "22222222-2222-4222-8222-222222222222" }; }, verifyAssertions: verify, verifyCalibration: async (session) => { expect(await session.page.getByLabel("Name").inputValue()).toBe("Capsule app"); expect(await session.page.getByLabel("Description").inputValue()).toBe("Prepared only in capsule"); calibrationBoundaryVerified = true; }, reconcile: async (session) => { await session.page.goto(`${origin}/safe`); return (await session.page.getByTestId("created").textContent()) === "Created" ? "succeeded" : "not_applied"; } }).execute(transaction(origin));
