...
import { resolveTarget, resolveTargetLocator } from "@scry/praxis";
import {
...
const clickTarget = (name: string, overrides: Partial<InteractionTargetIntent> = {}) =>
  target(name, {
...
const fillScenario = (
  ...
  run: async (page) => {
    ...
    expect(
      await page
        .locator(exactSelector)
        .inputValue()
        .catch(() => page.locator(exactSelector).textContent()),
    ).toBe(value);
  },
});
...
const scenarios: Scenario[] = [
  ...
  fillScenario(
    4,
    "placeholder-only weak markup",
    `<input id="email" placeholder="Email">`,
    fillTarget("Email", {
      preferredEvidence: identity("Email", { roles: ["textbox"], placeholders: ["Email"] }),
    }),
    "#email",
  ),
  fillScenario(
    5,
    "unassociated visual label with geometry anchor",
    `<form><div class="field-label">Email address</div><input id="email" placeholder="name@example.test"></form>`,
    fillTarget("Email address", {
      preferredEvidence: identity("Email address", {
        roles: ["textbox"],
        placeholders: ["name@example.test"],
        visual: { sources: ["geometry"], expectedText: "Email address", protectedUse: false },
      }),
      scope: { kind: "form" },
      confidence: {
  ...
  {
    id: 6,
    name: "OCR is forbidden as required evidence for a password control",
    body: `<form><div class="ocr-label">Password</div><input id="password" type="password" placeholder="Enter your password"></form>`,
    run: async (page) => {
      await expectCode(
        fillGroundedTarget(
          page,
          fillTarget("Password", {
            preferredEvidence: identity("Password", {
              roles: ["textbox"],
              placeholders: ["Enter your password"],
              inputTypes: ["password"],
              visual: { sources: ["ocr"], expectedText: "Password", protectedUse: false },
            }),
            scope: { kind: "form" },
            risk: "authentication",
            confidence: {
              requiredFamilies: ["visual", "native_control"],
              minimum: 0.55,
              minimumMargin: 0.05,
              minimumFamilyCount: 3,
            },
  ...
  fillScenario(
    7,
    "nested native label",
    `<label>Account name <span><input id="account"></span></label>`,
    fillTarget("Account name"),
    "#account",
  ),
  fillScenario(
    8,
    "content-editable custom textbox",
    `<div id="bio" role="textbox" aria-label="Biography" contenteditable="true"></div>`,
  ...
  fillScenario(
    10,
    "hidden duplicate rejected",
    `<input aria-label="Email" style="display:none"><input id="visible" aria-label="Email">`,
    fillTarget("Email"),
    "#visible",
  ),
  fillScenario(
    11,
    "disabled duplicate rejected",
    `<input aria-label="Email" disabled><input id="enabled" aria-label="Email">`,
    fillTarget("Email"),
    "#enabled",
  ),
  fillScenario(
    12,
    "readonly duplicate rejected",
    `<input aria-label="Email" readonly><input id="editable" aria-label="Email">`,
    fillTarget("Email"),
    "#editable",
  ),
  fillScenario(
    13,
    "duplicate forms resolved by named scope",
    `<form aria-label="Search"><input aria-label="Query"></form><form aria-label="Support"><input id="support-query" aria-label="Query"></form>`,
    fillTarget("Query", { scope: { kind: "form", name: "Support" } }),
    "#support-query",
  ),
  fillScenario(
    14,
    "dialog scope selects modal field",
    `<input aria-label="Name"><dialog open aria-label="Create user"><input id="modal-name" aria-label="Name"></dialog>`,
    fillTarget("Name", { scope: { kind: "dialog", name: "Create user" } }),
    "#modal-name",
  ),
  fillScenario(
    15,
    "field-group scope selects shipping field",
    `<fieldset><legend>Billing</legend><label>Postcode<input></label></fieldset><fieldset><legend>Shipping</legend><label>Postcode<input id="shipping"></label></fieldset>`,
    fillTarget("Postcode", { scope: { kind: "field_group", name: "Shipping" } }),
    "#shipping",
  ),
  fillScenario(
    16,
    "reversed visual label order",
    `<form><input id="code" placeholder="Account code"><div>Account code</div></form>`,
    fillTarget("Account code", {
      preferredEvidence: identity("Account code", {
        roles: ["textbox"],
        placeholders: ["Account code"],
        visual: { sources: ["geometry"], expectedText: "Account code", protectedUse: false },
      }),
      scope: { kind: "form" },
    }),
    "#code",
  ...
  clickScenario(
    17,
    "native button",
    `<button onclick="effect.textContent='clicked-17'">Save</button><output id="effect"></output>`,
    clickTarget("Save"),
  ),
  clickScenario(
    18,
    "link implemented as control",
    `<a href="#continued" aria-label="Continue" onclick="effect.textContent='clicked-18'">Go</a><output id="effect"></output>`,
    target("Continue", { preferredEvidence: identity("Continue", { roles: ["link"] }) }),
  ),
  clickScenario(
    19,
    "ARIA button custom control",
    `<div role="button" tabindex="0" aria-label="Save" onclick="effect.textContent='clicked-19'">S</div><output id="effect"></output>`,
    clickTarget("Save"),
  ),
  clickScenario(
    20,
    "icon-only button",
    `<button aria-label="Copy" onclick="effect.textContent='clicked-20'"><svg data-icon="copy"><title>Copy</title></svg></button><output id="effect"></output>`,
    clickTarget("Copy", {
      preferredEvidence: identity("Copy", {
        roles: ["button"],
  ...
  clickScenario(
    21,
    "deeply nested button text",
    `<button onclick="effect.textContent='clicked-21'"><span><strong>Continue</strong></span></button><output id="effect"></output>`,
    clickTarget("Continue"),
  ),
  {
    id: 22,
    ...
    run: async (page) => {
      await checkGroundedTarget(
        ...
        target("Accept terms", {
          ...
          preferredEvidence: identity("Accept terms", { roles: ["checkbox"] }),
        }),
        true,
      );
      ...
      expectCode(
        checkGroundedTarget(
          ...
          target("Notifications", {
            requiredCapabilities: ["toggleable"],
            preferredEvidence: identity("Notifications", { roles: ["checkbox"] }),
          }),
          true,
        ),
    ...
    run: async (page) => {
      await selectGroundedTarget(
        ...
        target("Country", {
          ...
          preferredEvidence: identity("Country", { roles: ["combobox"] }),
        }),
        "gh",
      );
  ...
  },
  {
    ...
    run: async (page) =>
      expectCode(
        resolveTarget(page, clickTarget("Save", { scope: { kind: "form", name: "Missing" } })),
  ...
  },
  {
    ...
    run: async (page) =>
      expectCode(resolveTarget(page, fillTarget("Password")), "NO_CAPABILITY_COMPATIBLE_CONTROL"),
  },
  {
    id: 30,
  ...
  },
  {
    ...
    run: async (page) => {
      const result = await clickGroundedTarget(
        page,
        target("Add", {
          requiredCapabilities: ["coordinate_action", "pointer_activatable"],
          preferredEvidence: identity("Add", {
            roles: ["button"],
    ...
    run: async (page) =>
      expectCode(
        resolveTarget(
          ...
          target("Delete", {
            requiredCapabilities: ["coordinate_action", "pointer_activatable"],
            preferredEvidence: identity("Delete", {
              roles: ["button"],
  ...
  },
  {
    ...
    run: async (page) => {
      ...
      page.on("response", async (response) => {
        if (new URL(response.url()).pathname === "/api/token")
          responses.push({
            origin,
      ...
      const result = await builtInAdapterRegistry.execute(
        "gauntlet.network",
        {
          page,
          allowedOrigins: [origin],
          protectedInterval: true,
          registerSecret: (value) => registered.push(value),
          networkResponses: responses,
        },
        { origin, method: "GET", path: "/api/token", jsonPointer: "/token" },
      );
...
describe("cohort A: 34 real-HTTP capability-grounding scenarios", () => {
  ...
  for (const scenario of scenarios) {
    it(
      `${String(scenario.id).padStart(3, "0")} ${scenario.name}`,
      async () => {
