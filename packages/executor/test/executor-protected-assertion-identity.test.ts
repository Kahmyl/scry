...
describe("executor protected assertion persistence identity", () => {
  it("assigns distinct ordinals and transaction ids across protected assertion phases", async () => {
    ...
    const server = createServer((request, response) => {
      ...
      response.end(
        `<label>Name<input></label><button>Generate</button><output aria-label="Public identifier"></output><output aria-label="Protected generated value" data-scry-redacted="true"></output><section role="region" aria-label="Created" hidden>Created</section><script>document.querySelector('button').onclick=()=>{document.querySelector('[aria-label="Public identifier"]').textContent=${JSON.stringify(publicValue)};document.querySelector('[aria-label="Protected generated value"]').textContent=${JSON.stringify(protectedValue)};document.querySelector('[aria-label="Created"]').hidden=false}</script>`,
      );
    });
    servers.push(server);
    ...
    const report = await executePlan({
      plan: currentPlanSchema.parse({
        ...
        steps: [
          {
            id: "open-safe",
            title: "Open safe context",
            action: { type: "navigate", url: `${origin}/safe` },
            assertions: [],
            evidence: [],
            onFailure: "stop",
            captureIntent: "final",
      ...
      publicValueCapture: async ({ value }) => {
        expect(value).toBe(publicValue);
    ...
    expect(
      ...
      JSON.stringify({
        error: report.error,
        steps: report.steps.map(({ id, status, action }) => ({ id, status, action })),
      }),
    ).toBe("passed");
    const assertionResults = results.filter((result) =>
      result.operationId.startsWith("assertion-"),
    );
    ...
    expect(new Set(assertionResults.map((result) => result.transactionId)).size).toBe(
      assertionResults.length,
    );
    expect(
      assertionResults.every(
        (result) => result.stepId === "protected" && result.status === "succeeded",
      ),
    ).toBe(true);
  }, 30_000);
});
...
function protectedTransaction(origin: string): ProtectedTransaction {
  ...
  const output = (concept: string, classification: "public" | "unknown_secret") => ({
  ...
  return {
    ...
    preparation: {
      ...
      actions: [
        { type: "fillPublicInput", input: "name", target: name, effect: "replayable_setup" },
        {
          type: "assertion",
          assertion: { type: "enabled", target: generate },
          effect: "read_only",
        },
      ],
      assertions: [
        { type: "fieldValueMatchesInput", target: name, input: "name" },
        { type: "visible", target: generate },
      ],
    },
    mutation: {
      action: {
        type: "click",
        target: generate,
        expectedEffect: { type: "new_region", target: created },
      },
      kind: "one_time",
      ...
      outputs: [
        {
          ...
          acquisition: output("Public identifier", "public"),
          storage: { name: "Public identifier", scope: "run" },
        },
        {
          classification: "protected",
          reference: "generatedSecret",
          acquisition: output("Protected generated value", "unknown_secret"),
    ...
    acquisitionReadiness: {
      ...
      valueIntent: intent("Protected generated value", "value"),
      ...
      strategies: [
        {
          ...
          assertions: [{ type: "visible", target: safe }],
          continueAtStepId: "complete",
        },
      ],
    },
  };
...
function intent(
  concept: string,
  role: "textbox" | "button" | "region" | "value",
  ...
  return {
    ...
    risk: role === "value" ? "read_only" : "ordinary",
