...
const checkpoint = {
  ...
  verificationAssertions: [{ type: "url" as const, expected: "/safe", match: "path" as const }],
  continueAtStepId: "continue",
  maxRestorations: 1 as const,
  state: { cookies: true as const, localStorage: true as const, indexedDb: true },
};
...
describe("deterministic checkpoint recovery", () => {
  it("destroys uncertainty before creating and verifying the restored context", async () => {
    ...
    const store = {
      establish: vi.fn(async (input) => {
        persisted = input;
      }),
      claim: vi.fn(async () => ({
        payload: (persisted as { payload: unknown }).payload,
        bindingFingerprint: (persisted as { bindingFingerprint: string }).bindingFingerprint,
      })),
    ...
    const original = {
      ...
      close: vi.fn(async () => {
        order.push("old-context-destroyed");
      }),
    } as unknown as BrowserContext;
    await coordinator.establish(original, checkpoint);
    const page = {
      goto: vi.fn(async () => {
        order.push("restored-navigation");
      }),
      url: vi.fn(() => "https://example.test/safe"),
    } as unknown as Page;
    await coordinator.restore(
      checkpoint,
      original,
      async () => {
        order.push("new-context-created");
        return { context: { close: vi.fn() } as unknown as BrowserContext, page };
      },
      async () => {
        order.push("boundary-verified");
      },
    );
    expect(order).toEqual([
      "old-context-destroyed",
      "new-context-created",
      "restored-navigation",
      "boundary-verified",
    ]);
  ...
  it("destroys a claimed checkpoint and aborts when binding verification fails", async () => {
    ...
    await expect(coordinator.restore(checkpoint, original, vi.fn(), vi.fn())).rejects.toMatchObject(
...
describe("calibration and built-in adapters", () => {
  it("produces stable fingerprints for reordered structural sets and changes on drift", () => {
    ...
    expect(structureFingerprint(base)).toBe(
      structureFingerprint({ ...base, containers: ["main", "dialog"] }),
    );
    expect(structureFingerprint(base)).not.toBe(
      structureFingerprint({ ...base, testIds: ["changed"] }),
    );
    expect(structureFingerprint(base)).not.toBe(
  ...
  it("extracts an exact configured network value only inside protection", async () => {
    ...
    const context = {
      ...
      registerSecret: (value: string) => secrets.push(value),
      networkResponses: [
        {
          origin: "https://example.test",
          method: "POST",
    ...
    await builtInAdapterRegistry.execute("gauntlet.network", context, {
      origin: "https://example.test",
      method: "POST",
      path: "/issue",
      jsonPointer: "/token",
    ...
    await expect(
      builtInAdapterRegistry.execute(
        "gauntlet.network",
        { ...context, protectedInterval: false },
        { origin: "https://example.test", method: "POST", path: "/issue", jsonPointer: "/token" },
      ),
    ).rejects.toMatchObject({ code: "ADAPTER_REQUIRES_PROTECTED_INTERVAL" });
  });
  it("does not permit arbitrary adapter registration collisions", () => {
    const registry = new AdapterRegistry();
    const adapter = {
      id: "fixed",
      capability: "safe_exit" as const,
      validate: () => ({}),
      suppressedChannels: [],
      timeoutMs: 100,
      execute: async () => ({ code: "ADAPTER_COMPLETED" as const, durationMs: 0 }),
    };
