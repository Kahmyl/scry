...
async function fixture() {
  ...
  const permit: VeilCapturePermit = {
    schemaVersion: VEIL_CONTRACT_VERSION,
    token: `veil_capture_${"a".repeat(43)}`,
  ...
  const unregister = registerVeilEvidenceAdmission({
    ...
    visualAdmission: (candidate) => {
      ...
      return {
        capturePermitDigest: "d".repeat(64),
        maskDigest: candidate.maskDigest,
...
describe("Veil artifact admission", () => {
  it("requires explicit classification and pre-capture visual sanitation", async () => {
    ...
    try {
      await expect(availableArtifact("screenshot", "image/png", file)).rejects.toThrow(
        "VEIL_EVIDENCE_CLASSIFICATION_REQUIRED",
      );
      await expect(
        availableArtifact("screenshot", "image/png", file, undefined, {
          classification: "unknown",
        }),
      ).rejects.toThrow("VEIL_EVIDENCE_CLASSIFICATION_REFUSED");
      await expect(
        availableArtifact("screenshot", "image/png", file, undefined, { classification: "public" }),
  ...
  it("admits bytes only with the recognized pre-capture visual attestation", async () => {
    ...
    try {
      ...
      expect(artifact.observation).toMatchObject({
        veilManifest: { classification: "public" },
        veilSanitation: { stage: "pre_capture" },
      });
