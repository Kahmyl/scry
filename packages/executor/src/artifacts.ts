...
type AdmissionRuntime = { root: string; authority: VeilAuthority; admissionKey: string; context: () => VeilLeaseRequest["context"]; visualAdmission?: (permit: VeilCapturePermit) => { capturePermitDigest: string; maskDigest: string; documentEpoch: number }; videoAdmission?: (finalization: VeilVideoSegmentFinalization) => VeilVideoSegmentFinalization };
const admissionRuntimes = new Map<string, AdmissionRuntime>();
type VeilSanitationEvidence = {
  stage: "pre_capture" | "post_capture";
  method: "veil_capture_permit" | "veil_init_style_and_dynamic_masking" | "veil_dynamic_masking_and_protected_recording_gaps" | "SecretRedactor.redact" | "SecretRedactor.redactValue" | "sanitizeTraceArchive";
  attestedAt: string;
...
export function registerVeilEvidenceAdmission(runtime: AdmissionRuntime): () => void {
  ...
  return () => { admissionRuntimes.delete(root); };
}
...
export async function availableArtifact(
  ...
  if (classification === "secret" || classification === "unknown") throw new Error("VEIL_EVIDENCE_CLASSIFICATION_REFUSED");
  ...
  if (kind === "screenshot") {
    if (!evidence.capturePermit || !runtime.visualAdmission) throw new Error("VEIL_CAPTURE_PERMIT_BINDING_REQUIRED");
    sanitation = { stage: "pre_capture", method: "veil_capture_permit", ...runtime.visualAdmission(evidence.capturePermit), attestedAt: new Date().toISOString() };
  }
  if (kind === "video") {
    if (!evidence.videoFinalization || !runtime.videoAdmission) throw new Error("VEIL_VIDEO_CAPTURE_PERMIT_REQUIRED");
    const finalization = runtime.videoAdmission(evidence.videoFinalization);
    sanitation = { stage: "pre_capture", method: "veil_dynamic_masking_and_protected_recording_gaps", attestedAt: finalization.finalizedAt, documentEpoch: finalization.documentEpoch, checkpointChainDigest: finalization.checkpointChainDigest, checkpointCount: finalization.checkpointCount };
  }
  if ((kind === "screenshot" || kind === "video") && sanitation?.stage !== "pre_capture") throw new Error("VEIL_VISUAL_PRECAPTURE_SANITATION_REQUIRED");
  const admittedEvidence: { classification: "public" | "sensitive"; sanitation?: VeilSanitationEvidence } = {
    classification, ...(sanitation ? { sanitation } : {}),
  };
  const channel = artifactChannel(kind);
  const request: VeilLeaseRequest = { context: runtime.context(), operation: "admit_evidence", channel, classification, scope: "document" };
  const { decision, lease } = runtime.authority.issueLease(request);
  return admittedArtifact({ authority: runtime.authority, request, decision, lease, admissionKey: runtime.admissionKey, channel, kind, contentType, filePath, evidence: admittedEvidence, ...(relativePath ? { relativePath } : {}) });
}

function artifactChannel(kind: Artifact["kind"]): VeilEvidenceChannel {
  return ({ screenshot: "screenshot", video: "video", dom: "dom", network: "network", console: "console", trace: "trace", report: "report" } as const)[kind];
}
...
async function admittedArtifact(input: {
  ...
  if (input.decision.policyDigest !== validated.policyDigest || input.decision.disposition !== validated.disposition) {
  ...
  const sanitation = input.evidence.sanitation ?? { stage: "not_required", method: "classified_public_at_source", attestedAt: new Date().toISOString() };
