...
export interface PraxisTransactionAdapter<TTarget = unknown> {
  ...
  ground(request: PraxisRequest, signal: AbortSignal): Promise<{ target: TTarget; resolution: PraxisResolution }>;
...
export class PraxisTransactionCoordinator<TTarget = unknown> {
  ...
  async execute(request: PraxisRequest, signal: AbortSignal): Promise<PraxisResult> {
    ...
    try {
      ...
      target = grounded.target; resolution = grounded.resolution;
      ...
      const timing = timingFrom(durations, performance.now() - started);
