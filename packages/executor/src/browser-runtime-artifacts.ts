...
export function inspectBrowserRuntimeArtifacts(namedSources: Record<string, string>) {
  const diagnostics = Object.entries(namedSources).flatMap(([subsystem, source]) =>
    ["__name", "[native code]"].filter((token) => source.includes(token)).map((token) => ({
      code: "BROWSER_RUNTIME_FREE_VARIABLE", subsystem, message: `Browser artifact contains forbidden token ${token}.`,
    })),
  );
  return {
    healthy: diagnostics.length === 0,
    runtimeHash: createHash("sha256").update(stable(namedSources)).digest("hex"),
    capabilityManifestHash: createHash("sha256").update(stable({ artifacts: Object.keys(namedSources).sort(), version: 1 })).digest("hex"),
    diagnostics,
  };
}
