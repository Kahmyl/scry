...
import { browserObservationRuntimeHealth } from "./grounding.js";
import { executePraxisConsumer } from "./praxis-consumer.js";
...
export async function probeFlowPlan(input:{plan:CurrentPlan;level:ProbeExecutionLevel;policy:ExecutionPolicy;browserChannel:string;outputDirectory:string;secretResolver?:(reference:string)=>Promise<string>;captureBrowserState?:(state:BrowserStorageState)=>void|Promise<void>}):Promise<ProbeExecutionResult>{
  ...
  try{
    for(const [stepIndex,step] of input.plan.steps.entries()){if(step.action.type==="navigate"){await page.goto(step.action.url,{waitUntil:"domcontentloaded"}).catch(error=>diagnostics.push({code:"PROBE_NAVIGATION_FAILED",stepId:step.id,message:safe(error)}));continue;}const target=actionTarget(step.action);if(target)await inspectTarget(page,step.id,"action",stepIndex,target,input.plan.allowedOrigins,targets,diagnostics);for(const [conditionIndex,condition] of (step.after?.conditions??[]).entries()){const candidate="target"in condition?condition.target:undefined;if(candidate)await inspectTarget(page,step.id,"readiness",conditionIndex,candidate,input.plan.allowedOrigins,readiness,diagnostics);}}
...
async function inspectTarget(page:import("playwright").Page,stepId:string,channel:string,ordinal:number,target:InteractionTargetIntent,allowedOrigins:string[],output:Array<Record<string,unknown>>,diagnostics:Array<Record<string,unknown>>){const result=await executePraxisConsumer({page,intent:target,operation:{type:"inspect"},context:{stepId,channel:"probe",ordinal,allowedOrigins,timeoutMs:10_000},signal:new AbortController().signal});if(result.status==="succeeded")output.push({stepId,channel,status:"resolved",confidence:result.resolution.confidence,confidenceMargin:result.resolution.runnerUpMargin,fingerprint:result.resolution.target,strategy:result.resolution.strategy});else diagnostics.push({code:result.code,stepId,channel,provenance:result.provenance,retry:result.retry,mutationOutcome:result.mutationOutcome,safeActions:result.safeActions});}
