...
export async function probeFlowPlan(input: {
  ...
  if (input.level !== "inspection") {
    ...
    return {
      ...
      targets: report.steps.map((step) => ({ stepId: step.id, status: step.action.status })),
      readiness: report.steps.map((step) => ({ stepId: step.id, ...step.readiness })),
      diagnostics,
      pageFingerprint: hash({
        state: report.state,
        steps: report.steps.map((step) => [step.id, step.action.status, step.readiness?.status]),
      }),
      execution: { state: report.state, outcomeClassification: report.outcomeClassification },
    };
  }
  ...
  try {
    for (const [stepIndex, step] of input.plan.steps.entries()) {
      if (step.action.type === "navigate") {
        await page.goto(step.action.url, { waitUntil: "domcontentloaded" }).catch((error) =>
          diagnostics.push({
...
async function inspectTarget(
  ...
  const result = await executePraxisConsumer({
    ...
    context: { stepId, channel: "probe", ordinal, allowedOrigins, timeoutMs: 10_000 },
