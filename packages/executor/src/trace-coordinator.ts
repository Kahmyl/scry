...
import type { Artifact, PrivacyFailure, RecordingTimelineEntry, SafeResumeBoundary } from "@scry/contracts";
...
export class TraceCoordinator implements PrivacyCollector {
  readonly name = "trace";
  private active: { id: string; startedAt: string; reason: "run_started" | "safe_resume"; path: string; relativePath: string } | undefined;
  ...
  start(reason: "run_started" | "safe_resume") {
    return this.serial(async () => {
      ...
      const id = randomUUID();
      const relativePath = path.posix.join("trace", `segment-${String(this.entries.length).padStart(4, "0")}-${id}.zip`);
      const target = path.join(this.options.outputDirectory, ...relativePath.split("/"));
      await mkdir(path.dirname(target), { recursive: true });
      await bounded(this.options.context.tracing.start({ screenshots: true, snapshots: true, sources: false }));
    ...
    return this.serial(async () => {
      ...
      try {
        ...
        if (quarantine || this.sealed) {
          ...
          const artifact: Artifact = { id: randomUUID(), kind: "trace", availability: "destroyed", privacyClassification: "uncertain", failureProvenance: "privacy", reasonCode: "PRIVACY_SEALED", contentType: "application/zip", observation: { reasonCode: "PRIVACY_SEALED", bytesDestroyed: true } };
        ...
        this.sealed = true;
        this.entries.push({ type: "trace_segment", id: active.id, sequence: this.entries.length, startedAt: active.startedAt, endedAt, reason: active.reason, status: "failed", privacyStatus: "quarantined", failureCode: "TRACE_STOP_FAILED" });
  ...
  private serial<T>(work: () => Promise<T>) { const next = this.transition.then(work, work); this.transition = next.then(() => undefined, () => undefined); return next; }
}

async function bounded<T>(work: Promise<T>) {
  let timer: NodeJS.Timeout | undefined;
  try { return await Promise.race([work, new Promise<never>((_, reject) => { timer = setTimeout(() => reject(new Error("TRACE_TRANSITION_TIMEOUT")), 5_000); })]); }
