...
export class VeilClipboardCollector extends VeilChannelCollector {
  ...
  constructor(private readonly page: Page, private readonly attempts = 3) { super("clipboard"); clipboardCollectors.set(page, this); }

  markProtectedClipboardTouched(): void { this.protectedClipboardTouched = true; }
  ...
  override async finalize(): Promise<void> {
    if (this.protectedClipboardTouched) {
      ...
      if (failure) throw new VeilClipboardCleanupError(`Veil could not verify protected clipboard destruction: ${failure instanceof Error ? failure.message : String(failure)}`);
    }
