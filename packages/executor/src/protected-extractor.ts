...
export async function acquireValue(page: Page, acquisition: AcquisitionIntent, timeoutMs: number) {
  ...
  const diagnostics: CandidateState[] = acquisition.permittedMethods.map((method, candidate) => ({ candidate, method, attempts: 0, durationMs: 0, containerResolved: false, matchCount: "unknown", visibility: "unknown", accessibility: "unknown" }));
  while (Date.now() < deadline) {
    for (const [index, method] of acquisition.permittedMethods.entries()) {
      if (Date.now() >= deadline) break;
      const diagnostic = diagnostics[index]!; const started = Date.now(); diagnostic.attempts += 1; diagnostic.firstAttemptedAt ??= new Date(started).toISOString();
      try {
        const result = await resolveTarget(page, acquisition.target);
        diagnostic.containerResolved = true; diagnostic.matchCount = "one"; diagnostic.visibility = "visible"; diagnostic.accessibility = "available";
        ...
        if (error instanceof GroundingError) { diagnostic.matchCount = error.code === "TARGET_AMBIGUOUS" ? "many" : "none"; diagnostic.containerResolved = error.code !== "TARGET_NOT_FOUND"; }
...
async function acquireByMethod(page: Page, target: Locator, method: AcquisitionIntent["permittedMethods"][number], intent: AcquisitionIntent["target"]): Promise<string> {
  switch (method) {
    case "input_value": return target.inputValue();
    case "dom_text": return target.textContent().then((value) => value ?? "");
    case "semantic_field_value": return target.evaluate((element) => element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement || element instanceof HTMLSelectElement ? element.value : element.textContent ?? "");
    case "copy_control": {
      const group = target.locator("xpath=ancestor::*[self::fieldset or @role='group' or self::div][1]");
    ...
    case "focused_keyboard_selection": { await target.focus(); await page.keyboard.press(process.platform === "darwin" ? "Meta+A" : "Control+A"); const value = await target.evaluate((element) => element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement ? element.value.substring(element.selectionStart ?? 0, element.selectionEnd ?? element.value.length) : getSelection()?.toString() ?? ""); await page.keyboard.press("Escape").catch(() => undefined); return value; }
    case "approved_network_field": throw new AcquisitionError("NETWORK_ADAPTER_REQUIRED");
    case "application_adapter": throw new AcquisitionError("APPLICATION_ADAPTER_REQUIRED");
    case "secure_user_assistance": throw new AcquisitionError("SECURE_ASSISTANCE_REQUIRED");
    case "protected_visual_reading": return acquireProtectedVisualText(page, intent);
  }
}
