...
export type PraxisProviderTiming = { provider: string; durationMs: number; outcome: PraxisProviderOutcome };
...
export const PRAXIS_PROVIDER_CATALOG = Object.freeze([
  { id: "native-control", version: 1, families: ["native_control"], cost: "constant", privacy: "public_dom", concurrent: true, maximumWork: 5_000 },
  { id: "computed-accessibility", version: 1, families: ["accessibility"], cost: "low", privacy: "accessibility", concurrent: true, maximumWork: 5_000 },
  { id: "textual-identity", version: 1, families: ["textual"], cost: "low", privacy: "public_dom", concurrent: true, maximumWork: 5_000 },
  { id: "structural-relationship", version: 1, families: ["structural"], cost: "low", privacy: "public_dom", concurrent: true, maximumWork: 5_000 },
  { id: "geometry-hit-test", version: 1, families: ["structural"], cost: "low", privacy: "public_dom", concurrent: true, maximumWork: 5_000 },
  { id: "visual-icon-ocr-canvas", version: 1, families: ["visual"], cost: "high", privacy: "visual", concurrent: false, maximumWork: 100 },
  { id: "runtime-behavior", version: 1, families: ["runtime"], cost: "medium", privacy: "public_dom", concurrent: false, maximumWork: 500 },
  { id: "historical-fingerprint", version: 1, families: ["historical"], cost: "constant", privacy: "public_dom", concurrent: true, maximumWork: 1 },
  { id: "expected-effect-compatibility", version: 1, families: ["runtime"], cost: "constant", privacy: "public_dom", concurrent: true, maximumWork: 1 },
  { id: "application-adapter", version: 1, families: ["runtime"], cost: "medium", privacy: "protected", concurrent: false, maximumWork: 10 },
] as const);
...
function pageId(page: Page) { let value = pageIds.get(page); if (!value) { value = `page-${++pageSequence}`; pageIds.set(page, value); } return value; }
...
function epochRuntime() {
  const state = globalThis as typeof globalThis & { __scryPraxisEpoch?: number; __scryPraxisObserver?: MutationObserver };
  ...
  state.__scryPraxisObserver = new MutationObserver(() => {
    if (scheduled) return;
    scheduled = true;
    queueMicrotask(() => { state.__scryPraxisEpoch = (state.__scryPraxisEpoch ?? 0) + 1; scheduled = false; });
  });
  state.__scryPraxisObserver.observe(document.documentElement, { attributes: true, childList: true, subtree: true, characterData: true });
}
...
export class PraxisTargetHandle {
  ...
  toJSON() { return { pageId: this.pageId, frameId: this.frameId, documentEpoch: this.documentEpoch, runtimeVersion: this.runtimeVersion }; }
}

export class PraxisStaleTargetError extends Error { constructor() { super("PRAXIS_TARGET_CHANGED_BEFORE_ACTION"); this.name = "PraxisStaleTargetError"; } }
...
export class PraxisObservationCache {
  ...
  static set(page: Page, key: string, snapshot: PraxisObservationSnapshot) {
    let cache = observationCaches.get(page); if (!cache) { cache = new Map(); observationCaches.set(page, cache); }
...
export type PraxisGroundingScore = { families: Partial<Record<EvidenceFamily, number>>; familyCount: number; total: number };
export const PraxisGroundingPolicyV1 = Object.freeze({
  version: PRAXIS_GROUNDING_POLICY_VERSION,
  candidateLimit: 5_000,
  strongFamilyThreshold: .55,
  highRiskMinimum: .76,
  ordinaryMinimum: .58,
  highRiskMargin: .14,
  ordinaryMargin: .08,
  ...
  score(evidence: readonly CandidateEvidence[]): PraxisGroundingScore {
    ...
    for (const item of evidence) { const current = groups.get(item.correlationGroup); if (!current || item.score > current.score) groups.set(item.correlationGroup, item); }
    const families: Partial<Record<EvidenceFamily, number>> = {};
    for (const item of [...groups.values()].sort((a, b) => a.correlationGroup.localeCompare(b.correlationGroup))) families[item.family] = Math.max(families[item.family] ?? 0, item.score);
    const scores = Object.values(families);
    const familyCount = scores.filter((value) => value >= .55).length;
    const total = scores.length ? Math.min(1, scores.reduce((sum, value) => sum + value, 0) / Math.max(2, scores.length) + Math.min(.12, familyCount * .03)) : 0;
    return { families, familyCount, total };
  },
});

export async function runEvidenceProviders<T>(providers: readonly PraxisEvidenceProvider<T>[], context: T, allowedChannels: readonly string[], signal: AbortSignal) {
  const execute = async (provider: PraxisEvidenceProvider<T>) => {
    if (!allowedChannels.includes(provider.privacy)) return { provider, evidence: [] as readonly CandidateEvidence[], timing: { provider: provider.id, durationMs: 0, outcome: "forbidden" as const } };
    const started = performance.now();
    try { const evidence = provider.sanitize(await provider.observe(context, signal)); return { provider, evidence, timing: { provider: provider.id, durationMs: performance.now() - started, outcome: "succeeded" as const } }; }
    catch (error) { if (signal.aborted) throw error; return { provider, evidence: [] as readonly CandidateEvidence[], timing: { provider: provider.id, durationMs: performance.now() - started, outcome: "failed" as const } }; }
  };
