...
export type VeilVideoSegmentBinding = Pick<VeilContext, "browserContextId" | "pageId" | "frameId" | "documentEpoch">;
...
export class VeilVideoSegmentAuthority {
  ...
  constructor(
    ...
    if (!/^[a-f0-9]{64}$/.test(policyDigest)) throw new VeilVideoSegmentError("VEIL_POLICY_INVALID", "A valid policy digest is required");
    if (!Number.isInteger(ttlMs) || ttlMs < 1_000 || ttlMs > 1_800_000) throw new VeilVideoSegmentError("VEIL_VIDEO_TTL_INVALID", "Video segment TTL must be between 1 second and 30 minutes");
  }

  issue(segmentId: string, binding: VeilVideoSegmentBinding): VeilVideoSegmentPermit {
    assertBinding(binding);
    if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/.test(segmentId)) throw new VeilVideoSegmentError("VEIL_VIDEO_SEGMENT_ID_INVALID", "Video segment id is invalid");
  ...
  async checkpoint(page: Page, rawPermit: VeilVideoSegmentPermit, binding: VeilVideoSegmentBinding): Promise<VeilVideoMaskCheckpoint> {
    ...
    const checkpoint = veilVideoMaskCheckpointSchema.parse({ ...unsigned, checkpointDigest: digest(unsigned) });
    state.checkpoints.push(checkpoint);
    return Object.freeze({ ...checkpoint });
  }

  finalize(rawPermit: VeilVideoSegmentPermit, binding: VeilVideoSegmentBinding): VeilVideoSegmentFinalization {
    const state = this.requireActive(rawPermit, binding);
    const checkpointChainDigest = state.checkpoints.at(-1)?.checkpointDigest;
    if (!checkpointChainDigest) throw new VeilVideoSegmentError("VEIL_VIDEO_CHECKPOINT_REQUIRED", "A video segment cannot be finalized without a visual mask checkpoint");
    this.segments.delete(state.permit.token);
    const finalization = Object.freeze(veilVideoSegmentFinalizationSchema.parse({
      schemaVersion: VEIL_CONTRACT_VERSION,
      segmentId: state.permit.segmentId,
  ...
  consumeFinalization(raw: VeilVideoSegmentFinalization): VeilVideoSegmentFinalization {
    ...
    if (!stored || !sameFinalization(stored, finalization)) throw new VeilVideoSegmentError("VEIL_VIDEO_FINALIZATION_INVALID", "Video finalization is forged, unknown, or already consumed");
    this.finalized.delete(finalization.segmentPermitDigest);
    return Object.freeze({ ...stored });
  }

  revoke(permit: VeilVideoSegmentPermit): boolean { return this.segments.delete(permit.token); }

  private requireActive(rawPermit: VeilVideoSegmentPermit, binding: VeilVideoSegmentBinding): SegmentState {
    const permit = veilVideoSegmentPermitSchema.parse(rawPermit);
    const state = this.segments.get(permit.token);
    if (!state || !same(permit, state.permit)) throw new VeilVideoSegmentError("VEIL_VIDEO_PERMIT_INVALID", "Video segment permit is forged, revoked, finalized, or unknown");
    if (permit.policyDigest !== this.policyDigest) throw new VeilVideoSegmentError("VEIL_VIDEO_POLICY_STALE", "Video segment policy is stale");
    if (this.now() >= Date.parse(permit.expiresAt)) throw new VeilVideoSegmentError("VEIL_VIDEO_PERMIT_EXPIRED", "Video segment permit has expired");
    if (digest(binding) !== permit.contextDigest || digest(binding) !== digest(state.binding)) throw new VeilVideoSegmentError("VEIL_VIDEO_CONTEXT_MISMATCH", "Video segment permit does not match context or document epoch");
    return state;
  }
}

function assertBinding(binding: VeilVideoSegmentBinding) {
  if (!binding.browserContextId || !binding.pageId || !binding.frameId || !Number.isInteger(binding.documentEpoch) || binding.documentEpoch < 0) throw new VeilVideoSegmentError("VEIL_VIDEO_CONTEXT_INVALID", "Video segment binding is incomplete or invalid");
}
...
function same(left: VeilVideoSegmentPermit, right: VeilVideoSegmentPermit): boolean {
  const a = Buffer.from(stable(left)); const b = Buffer.from(stable(right));
  return a.length === b.length && timingSafeEqual(a, b);
}
function sameFinalization(left: VeilVideoSegmentFinalization, right: VeilVideoSegmentFinalization): boolean {
  const a = Buffer.from(stable(left)); const b = Buffer.from(stable(right));
  return a.length === b.length && timingSafeEqual(a, b);
}
