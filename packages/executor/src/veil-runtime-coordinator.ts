...
import { VEIL_CONTRACT_VERSION, type VeilCollectorAcknowledgement, type VeilCollectorPhase, type VeilFailure } from "@scry/contracts";
import { VeilRuntimeSession, type VeilRuntimeCollector } from "./veil-runtime-session.js";
...
export class VeilRuntimeCoordinator {
  ...
  constructor(
    ...
    this.runtime = new VeilRuntimeSession(runtimeCollectors, policyDigest, contextIdentity, async () => undefined, acknowledgementTimeoutMs);
  }
  ...
  prepare(operationId: string, preparation: PrivacyPreparation = { mode: "protected_recording_gap", videoMaskEstablished: false }) {
    return this.serial(async () => {
      ...
      try {
        ...
        throw new PrivacyTransitionError(`COLLECTOR_ARM_FAILED:${error instanceof Error ? error.message : String(error)}`);
      }
  ...
  beginProtected() { return this.serial(async () => { this.expect("ready_to_reveal"); await this.runtime.beginProtected(); await this.setState("protected"); }); }
  ...
  confirmSafeBoundary(boundary: SafeResumeBoundary) {
    return this.serial(async () => {
      ...
      try {
        ...
        this.currentState = "sealed";
        await this.emit({ state: "sealed", ...(this.operationId ? { operationId: this.operationId } : {}), code: "COLLECTOR_RESUME_FAILED" });
        throw new PrivacyTransitionError(`COLLECTOR_RESUME_FAILED:${error instanceof Error ? error.message : String(error)}`);
      }
  ...
  private async transitionCollector(collector: PrivacyCollector, phase: VeilCollectorPhase, operationId?: string) {
    if (phase === "prepare") { await collector.arm(operationId!, this.preparation); return this.requireCollectorState(collector, "prepared"); }
    if (phase === "suspend") { await collector.suspend(); return this.requireCollectorState(collector, "suspended"); }
    if (phase === "isolate") { await collector.isolate(); return this.requireCollectorState(collector, "isolated"); }
    if (phase === "resume") {
      if (!this.resumeBoundary) throw new PrivacyTransitionError("Veil resume requires a verified safe boundary");
      await collector.resume(this.resumeBoundary); return this.requireCollectorState(collector, "active");
    }
    if (phase === "seal") { await collector.seal({ code: "VEIL_RUNTIME_SEALED" }); return this.requireCollectorState(collector, "sealed"); }
    if (phase === "finalize") { await collector.finalize(); return this.requireCollectorState(collector, "finalized"); }
  }

  private requireCollectorState(collector: PrivacyCollector, expected: ReturnType<PrivacyCollector["state"]>["status"]) {
    const actual = collector.state().status;
    if (actual !== expected) throw new PrivacyTransitionError(`VEIL_COLLECTOR_STATE_MISMATCH:${collector.name}:${expected}:${actual}`);
  }
...
function acknowledgement(collectorId: string, phase: VeilCollectorPhase, stateVersion: number, operationId?: string): VeilCollectorAcknowledgement {
  return { schemaVersion: VEIL_CONTRACT_VERSION, collectorId, phase, stateVersion, ...(operationId ? { operationId } : {}), acknowledgedAt: new Date().toISOString() };
}

function veilFailure(reason: PrivacyFailure): VeilFailure {
  return { schemaVersion: VEIL_CONTRACT_VERSION, code: reason.code.startsWith("VEIL_") ? reason.code : `VEIL_${reason.code}`, provenance: "runtime", retry: "unsafe", ...(reason.collector ? { collectorId: reason.collector } : {}) };
}
