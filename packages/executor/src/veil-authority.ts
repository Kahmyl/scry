...
export class VeilAuthorityError extends Error {
  ...
  constructor(
    readonly code: "VEIL_ORIGIN_FORBIDDEN" | "VEIL_CAPABILITY_REFUSED" | "VEIL_LEASE_INVALID" | "VEIL_LEASE_EXPIRED" | "VEIL_LEASE_CONTEXT_MISMATCH" | "VEIL_POLICY_STALE",
    message: string,
  ) { super(message); }
}
...
export class VeilAuthority {
  ...
  private readonly decisions = new Map<string, Readonly<{ disposition: VeilDisposition; reasonCode: string }>>();
  private decisionCacheHits = 0;
  private decisionCacheMisses = 0;

  constructor(snapshot: VeilPolicySnapshot, private readonly now: () => number = Date.now) {
    this.policy = veilPolicySnapshotSchema.parse(snapshot);
  }

  snapshot(): VeilPolicySnapshot { return this.policy; }
  ...
  decide(raw: VeilLeaseRequest): VeilDecision {
    ...
    if (!this.policy.allowedOrigins.includes(request.context.origin)) {
      throw new VeilAuthorityError("VEIL_ORIGIN_FORBIDDEN", "Origin is not allowed by the active Veil policy");
    }
  ...
  issueLease(raw: VeilLeaseRequest): { decision: VeilDecision; lease: VeilCapabilityLease } {
    ...
    if (decision.disposition === "suppress" || decision.disposition === "quarantine") {
      throw new VeilAuthorityError("VEIL_CAPABILITY_REFUSED", `Capability refused: ${decision.reasonCode}`);
    }
  ...
  validateLease(lease: VeilCapabilityLease, rawExpected: VeilLeaseRequest): VeilDecision {
    ...
    if (!stored || !safeTokenEqual(stored.lease.token, lease.token)) throw new VeilAuthorityError("VEIL_LEASE_INVALID", "Capability lease is invalid");
    if (stored.lease.policyDigest !== this.policy.digest || lease.policyDigest !== this.policy.digest) throw new VeilAuthorityError("VEIL_POLICY_STALE", "Capability lease policy is stale");
    if (this.now() >= Date.parse(stored.lease.expiresAt) || lease.expiresAt !== stored.lease.expiresAt) {
      this.leases.delete(lease.token);
      throw new VeilAuthorityError("VEIL_LEASE_EXPIRED", "Capability lease has expired");
    }
    if (!sameRequest(stored.request, expected)) throw new VeilAuthorityError("VEIL_LEASE_CONTEXT_MISMATCH", "Capability lease does not match the requested context and scope");
  ...
  invalidateContext(match: Partial<VeilLeaseRequest["context"]>): number {
    ...
    for (const [token, stored] of this.leases) {
      if (Object.entries(match).every(([key, value]) => stored.request.context[key as keyof VeilLeaseRequest["context"]] === value)) {
        this.leases.delete(token); invalidated += 1;
      }
    }
    return invalidated;
  }

  revoke(lease: VeilCapabilityLease): boolean { return this.leases.delete(lease.token); }

  cacheStats(): Readonly<{ hits: number; misses: number; entries: number; policyDigest: string }> {
    return Object.freeze({ hits: this.decisionCacheHits, misses: this.decisionCacheMisses, entries: this.decisions.size, policyDigest: this.policy.digest });
  }

  private findLease(token: string): StoredLease | undefined { return this.leases.get(token); }
}
...
function dispositionFor(request: VeilLeaseRequest, policy: VeilPolicySnapshot): VeilDisposition {
  ...
  if (request.classification === "sensitive") return request.channel === "screenshot" || request.channel === "video" ? "sanitize" : "suppress";
...
function channelEnabled(channel: VeilEvidenceChannel, policy: VeilPolicySnapshot): boolean {
  const mapping: Record<VeilEvidenceChannel, keyof VeilPolicySnapshot["controls"] | undefined> = {
    screenshot: "screenshots", video: "video", dom: "dom", accessibility: "accessibility",
    console: "diagnostics", page_error: "diagnostics", network: "network", event: undefined,
    report: undefined, metadata: undefined, trace: "trace", clipboard: "clipboard", download: "downloads",
  };
...
function structured(channel: VeilEvidenceChannel): boolean {
  return ["dom", "accessibility", "console", "page_error", "network", "event", "report", "metadata", "trace"].includes(channel);
}
...
function sameRequest(left: VeilLeaseRequest, right: VeilLeaseRequest): boolean { return JSON.stringify(left) === JSON.stringify(right); }
