...
export class CheckpointCoordinator {
  ...
  async establish(context: BrowserContext, checkpoint: Checkpoint) {
    ...
    await this.input.store.establish({
      checkpoint,
      payload,
      bindingFingerprint,
  ...
  async restore(
    ...
    try {
      ...
      if (claimed.bindingFingerprint !== this.binding(checkpoint))
        throw new Error("CHECKPOINT_BINDING_MISMATCH");
      await lifecycle?.restoring?.();
      restored = await createContext({
        storageState: claimed.payload,
        serviceWorkers: "block",
        acceptDownloads: false,
      });
      await restored.page.goto(checkpoint.restorationUrl, { waitUntil: "domcontentloaded" });
      if (!this.input.allowedOrigins.includes(new URL(restored.page.url()).origin))
        throw new Error("CHECKPOINT_ORIGIN_MISMATCH");
      ...
      await this.input.store
        .complete(checkpoint.id, "failed", safeCode(error))
        .catch(() => undefined);
      throw new CheckpointRestoreError(safeCode(error));
    }
  }
  private binding(checkpoint: Checkpoint) {
    return createHash("sha256")
      .update(
        JSON.stringify({
          runId: this.input.runId,
          flowRevisionId: this.input.flowRevisionId,
          environmentId: this.input.environmentId,
          allowedOrigins: [...this.input.allowedOrigins].sort(),
          checkpointId: checkpoint.id,
          restorationUrl: checkpoint.restorationUrl,
        }),
      )
      .digest("hex");
  }
...
function safeCode(error: unknown) {
  ...
  return /^[A-Z][A-Z0-9_]*$/.test(value) ? value : "CHECKPOINT_RESTORE_FAILED";
}
