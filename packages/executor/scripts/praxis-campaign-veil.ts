...
import { executePraxisConsumer, type PraxisConsumerInput } from "../src/praxis-consumer.js";
...
export function createPraxisCampaignVeil(campaignId: string, origins: readonly string[]) {
  const authority = new VeilAuthority(compileVeilPolicy({ profile: "balanced", allowedOrigins: [...new Set(origins.map((value) => new URL(value).origin))], leaseTtlMs: 60_000 }));
  return {
    authority,
    register(page: Page, contextId: string) {
      return registerPraxisVeilAuthority(page, { authority, userId: `${campaignId}-user`, environmentId: "campaign", browserContextId: `${campaignId}-${contextId}` });
    },
  };
}
