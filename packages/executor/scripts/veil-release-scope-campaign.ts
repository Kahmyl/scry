...
const channels: VeilEvidenceChannel[] = [
  "video",
  "trace",
  "screenshot",
  "dom",
...
const failureRequirements = [
  ...
  "timeout",
  ...cancellationStates.map((state) => `cancellation-${state}` as const),
  "retry",
  "concurrency",
  "crash-recovery",
] as const;
...
class Collector implements VeilRuntimeCollector {
  constructor(
    readonly id: string,
    private readonly fail?: VeilCollectorPhase,
    private readonly hang?: VeilCollectorPhase,
  ) {}
  async transition(
...
async function main() {
  for (const phase of ["prepare", "suspend", "isolate", "resume", "seal", "finalize"] as const)
    await check(`failure:collector-${phase}`, "failure-injection", () => collectorFailure(phase));
  ...
  for (const id of [
    ...
    await check(`failure:${id}`, "failure-injection", () => contextInvalidation(id));
  ...
  await check("failure:masking-runtime-loss", "failure-injection", () =>
    failClosedBoundary("masking-runtime-loss"),
  );
  ...
  for (const state of cancellationStates)
    await check(`failure:cancellation-${state}`, "failure-injection", () => cancellationAt(state));
  ...
  const report = {
    schemaVersion: 1,
    campaign: "veil-failure-boundaries",
    ...
    coverage: {
      failureInjection: failureRequirements,
      productionPathEvidence: {
        controlledReplicasAndRetainedCanaries: "veil-production-e2e",
        opaqueBrowserAndLifecycleSurfaces: "veil-adversarial",
        corpusAndEndurance: "veil-certification",
      },
      companionExecutableCampaigns: [
        "veil-certification",
        "veil-adversarial",
        "veil-production-e2e",
        "veil-public-application-qualification",
        "veil-performance",
      ],
    },
    durableEvidence: {
      safeScenarioIdentifiers: true,
      content: "digests-and-dispositions-only",
      reportDigest: digest(results),
    },
    qualification:
      failed.length === 0 && missing.length === 0
        ? "VEIL_FAILURE_BOUNDARIES_PASS"
        : "VEIL_FAILURE_BOUNDARIES_FAIL",
    readiness:
      failed.length === 0 && missing.length === 0
...
async function collectorFailure(phase: VeilCollectorPhase) {
  const runtime = new VeilRuntimeSession(
    [new Collector(`collector-${phase}`, phase)],
    "a".repeat(64),
    `phase-${phase}`,
    undefined,
    10,
  );
  if (phase === "prepare" || phase === "suspend" || phase === "isolate") {
    await rejects(runtime.prepare("operation"));
    assert(runtime.state() === "sealed", "transition failure did not seal");
    return;
  }
  if (phase === "resume") {
    await runtime.prepare("operation");
    await runtime.beginProtected();
    await rejects(runtime.resume());
    assert(runtime.state() === "sealed", "resume failure did not seal");
    return;
  }
  if (phase === "seal") {
    await rejects(runtime.seal(failure("VEIL_INJECTED_SEAL")));
    assert(runtime.state() === "sealed", "seal failure did not remain sealed");
    return;
  }
  await rejects(runtime.finalize());
}
async function policyChange() {
  const one = compileVeilPolicy({
    profile: "balanced",
    allowedOrigins: ["https://policy.invalid"],
  });
  const two = compileVeilPolicy({ profile: "private", allowedOrigins: ["https://policy.invalid"] });
  const authority = new VeilAuthority(one);
  const req = request("policy", "screenshot", "public", "https://policy.invalid");
  const lease = authority.issueLease(req).lease;
  authority.updatePolicy(two);
  await rejects(() => authority.validateLease(lease, req));
}
async function leaseExpiry() {
  let now = 1_000;
  const authority = new VeilAuthority(
    compileVeilPolicy({
      profile: "balanced",
      allowedOrigins: ["https://expiry.invalid"],
      leaseTtlMs: 1_000,
    }),
    () => now,
  );
  const req = request("expiry", "screenshot", "public", "https://expiry.invalid");
  const lease = authority.issueLease(req).lease;
  now = 2_001;
  await rejects(() => authority.validateLease(lease, req));
}
async function contextInvalidation(id: string) {
  const origin = "https://context.invalid";
  const authority = authorityFor(origin);
  const req = request(id, "screenshot", "public", origin);
  const lease = authority.issueLease(req).lease;
  authority.invalidateContext(
    id === "document-epoch"
      ? { documentEpoch: 1 }
      : id === "page-loss" || id === "navigation"
        ? { pageId: `page-${id}` }
        : { browserContextId: `context-${id}` },
  );
  await rejects(() => authority.validateLease(lease, req));
}
async function failClosedBoundary(id: string) {
  const runtime = new VeilRuntimeSession([new Collector(id)], "b".repeat(64), id);
  await runtime.prepare(id);
  await runtime.beginProtected();
  await runtime.seal(failure(`VEIL_${id.replaceAll("-", "_").toUpperCase()}`));
  assert(runtime.state() === "sealed", `${id} did not seal`);
}
async function storageFailure() {
  const root = mkdtempSync(path.join(tmpdir(), "veil-storage-failure-"));
  const blocked = path.join(root, "not-a-directory");
  writeFileSync(blocked, "occupied");
  const bytes = Buffer.from("safe-evidence");
  const proof = admissionProof(bytes);
  const store = new LocalArtifactStore(blocked, "release-scope-admission-key-at-least-32-bytes");
  await rejects(store.put("artifact.bin", bytes, proof));
  assert(!(await store.exists("artifact.bin")), "storage failure retained artifact");
}
async function admissionFailure() {
  const root = mkdtempSync(path.join(tmpdir(), "veil-admission-failure-"));
  const bytes = Buffer.from("safe-evidence");
  const store = new LocalArtifactStore(root, "release-scope-admission-key-at-least-32-bytes");
  await rejects(store.put("artifact.bin", bytes, { ...admissionProof(bytes), token: "forged" }));
  assert(!(await store.exists("artifact.bin")), "failed admission retained artifact");
}
function admissionProof(bytes: Uint8Array) {
  return signVeilEvidenceAdmission(
    {
      schemaVersion: VEIL_CONTRACT_VERSION,
      evidenceId: "release-scope-evidence",
      channel: "report",
      classification: "public",
      disposition: "allow",
      policyDigest: "a".repeat(64),
      decisionId: "release-scope-decision",
      contentDigest: createHash("sha256").update(bytes).digest("hex"),
      omissionIntervals: [],
      createdAt: new Date().toISOString(),
    },
    "release-scope-admission-key-at-least-32-bytes",
  );
}
async function timeoutFailure() {
  const runtime = new VeilRuntimeSession(
    [new Collector("timeout", undefined, "prepare")],
    "c".repeat(64),
    "timeout",
    undefined,
    5,
  );
  await rejects(runtime.prepare("timeout"));
  assert(runtime.state() === "sealed", "timeout did not seal");
}
async function cancellationAt(target: (typeof cancellationStates)[number]) {
  if (target === "finalized") {
    const finalized = new VeilRuntimeSession(
      [new Collector("cancel-finalized")],
      "d".repeat(64),
      "cancel-finalized",
    );
  ...
  let runtime!: VeilRuntimeSession;
  let requested = false;
  runtime = new VeilRuntimeSession(
    [new Collector(`cancel-${target}`)],
    "d".repeat(64),
    `cancel-${target}`,
    async (event) => {
      if (event.to === target && target !== "finalized") {
        requested = true;
        queueMicrotask(() => void runtime.cancel());
      }
    },
  );
  if (target === "normal") {
...
async function childProcessCrashRecovery() {
  ...
  assert(
    ...
    `child did not crash with SIGKILL: ${crashed.status}/${crashed.signal}: ${crashed.stderr?.toString().replaceAll(root, "<temporary-directory>").slice(0, 2_000)}`,
  );
  ...
  assert(
    crashFiles.every((entry) => entry === "events.jsonl"),
    `crashed executor left unadmitted artifact files: ${crashFiles.join(",")}`,
  );
  assert(
    !readFileSync(path.join(artifacts, "events.jsonl"), "utf8").includes("VEIL_CHILD_SECRET"),
    "crashed executor persisted protected bytes in safe events",
  );
  ...
  assert(
    restart.status === 0 && recovery.state === "normal" && recovery.runState === "passed",
    `replacement Veil runtime/executor did not recover sealed checkpoint: ${restart.stderr?.toString()}`,
  );
}

function authorityFor(origin: string) {
  return new VeilAuthority(compileVeilPolicy({ profile: "balanced", allowedOrigins: [origin] }));
}
...
function request(
  ...
  origin = `https://${id}.replica.invalid`,
): VeilLeaseRequest {
  return {
    context: {
      userId: "release-campaign",
      environmentId: "controlled",
      transactionId: `tx-${id}`,
      origin,
      browserContextId: `context-${id}`,
      pageId: `page-${id}`,
...
function failure(code: string) {
  return {
    ...
    retry: "unsafe" as const,
  };
}
async function rejects(operation: Promise<unknown> | (() => unknown)) {
  let rejected = false;
  try {
...
async function check(id: string, category: string, operation: () => Promise<void>) {
  try {
    await operation();
    results.push({
      id,
      category,
      status: "passed",
      evidenceDigest: digest({ id, category, disposition: "safe" }),
    });
  } catch (error) {
    results.push({
      id,
