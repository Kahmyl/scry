...
import { compileDefaultVeilPolicy, compileVeilPolicy, VeilPolicyCompilationError, veilPolicyDigest } from "../src/veil-policy.js";
...
describe("compileVeilPolicy", () => {
  it("is deterministic, canonical, and deeply immutable", () => {
    ...
    expect(veilPolicyDigest({ schemaVersion: a.schemaVersion, profile: a.profile, allowedOrigins: a.allowedOrigins, controls: a.controls, leaseTtlMs: a.leaseTtlMs })).toBe(a.digest);
  });

  it("uses the most restrictive setting across layers", () => {
    const result = compileVeilPolicy([
      { profile: "balanced", allowedOrigins: ["https://a.example", "https://b.example"], controls: { video: true }, leaseTtlMs: 10_000 },
      { profile: "private", allowedOrigins: ["https://b.example"], controls: { screenshots: false }, leaseTtlMs: 1_000 },
    ]);
  ...
  it("never permits custom controls to weaken the safety floor", () => {
    expect(() => compileVeilPolicy({ profile: "custom", allowedOrigins: ["https://a.example"], controls: { maskSensitiveVisuals: false } as never })).toThrow();
  });

  it("rejects layers with no common origin", () => {
    expect(() => compileVeilPolicy([{ allowedOrigins: ["https://a.example"] }, { allowedOrigins: ["https://b.example"] }])).toThrow(VeilPolicyCompilationError);
  });

  it("adapts the existing execution policy without weakening it", () => {
    const result = compileDefaultVeilPolicy({ allowedOrigins: ["https://a.example"], allowDownloads: false });
