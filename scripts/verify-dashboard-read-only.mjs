...
const bundle = (await Promise.all(files.map((file) => readFile(resolve(assets, file), "utf8")))).join("\n");

const forbidden = [
  "New Mission", "Edit Mission", "New Flow", "Edit Flow", "Rerun exact plan",
  "/rerun", "/objectives", "/flow-revisions",
];
...
if (violations.length || missing.length) {
  ...
  console.log(JSON.stringify({ status: "passed", checkedBundles: files.length, forbidden, retained }, null, 2));
}
