...
if (!outputPath || command.length === 0)
  throw new Error("usage: capture-readiness-report.mjs OUTPUT COMMAND [ARGS...]");
...
const reports = extractJsonObjects(stdout).sort(
  (left, right) => right.source.length - left.source.length,
);
if (result.status !== 0 || reports.length === 0) {
  process.stderr.write(result.stderr ?? "");
  throw new Error(
    `campaign failed or produced no JSON: ${command.join(" ")} (exit ${result.status})`,
  );
}
