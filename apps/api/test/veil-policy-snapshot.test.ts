...
describe("run-bound Veil policy snapshots", () => {
  it("makes persisted private preferences authoritative for capture", () => {
    const snapshot = snapshotVeilPolicy(executionPolicy, {
      profile: "private",
      allowedOrigins: ["https://example.test"],
    });
    ...
    expect(
      ...
      reasonCode: "VEIL_CHANNEL_DISABLED",
    });
  });

  it("does not mutate an existing run snapshot when later preferences change", () => {
    const original = snapshotVeilPolicy(executionPolicy, {
      profile: "private",
      allowedOrigins: ["https://example.test"],
    });
    const later = snapshotVeilPolicy(executionPolicy, {
