...
describe("Veil HTTP contract", () => {
  ...
  const call = (method: "GET" | "PATCH", payload?: unknown) =>
    (
      ...
      method,
      url: `/api/environments/${environmentId}/veil`,
      ...(payload ? { payload } : {}),
    });
  ...
  it("validates and routes a tightening request", async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(veil.tighten).toHaveBeenCalledWith(expect.anything(), environmentId, {
      profile: "minimal_capture",
      reasonCode: "VEIL_USER_REQUESTED_PRIVACY",
    });
  });
