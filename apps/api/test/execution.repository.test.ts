...
describe("protected execution credentials", () => {
  ...
  it("refuses compromised credentials before decryption", async () => {
    ...
    const query = vi.fn()
      .mockResolvedValueOnce({ rowCount: 0, rows: [] })
      .mockResolvedValueOnce({ rowCount: 1, rows: [{ projectId: "project-id", authorized: true }] })
      .mockResolvedValueOnce({ rowCount: 1, rows: [{ ...encrypted, securityStatus: "compromised" }] });
    const repository = new ExecutionRepository({ query } as never);
    await expect(repository.lookupCredential("run-id", "11111111-1111-4111-8111-111111111111"))
  ...
  it("resolves a captured run secret only inside its immutable run", async () => {
    ...
    const query = vi.fn().mockResolvedValueOnce({ rowCount: 1, rows: [{ ...encrypted, securityStatus: "active" }] });
    const repository = new ExecutionRepository({ query } as never);
    await expect(repository.resolveCredential("run-id", "11111111-1111-4111-8111-111111111111")).resolves.toBe("captured-secret");
  ...
  it("claims a protected mutation with a fenced compare-and-set transition", async () => {
    ...
    const repository = new ExecutionRepository({ query } as never);
    await expect(repository.transitionProtectedMutation("run-id", "lease-id", { operationId: "operation-id", fencingToken: 4, expected: "planned", next: "dispatching" })).resolves.toBe(true);
  ...
  it("stores run-scoped captured secrets encrypted", async () => {
    ...
    const repository = new ExecutionRepository({ query } as never);
    await repository.persistCapturedSecret({ runId: "run-id", operationId: "op", reference: "secret", name: "Secret", value: "canary-value", scope: "run" });
    expect(query.mock.calls[0]?.[0]).toContain("run_captured_secrets");
  ...
  it("scopes generated public aliases to the originating transaction run", async () => {
    ...
    await repository.persistGeneratedPublicValue({
      runId: "run-id", operationId: "issue", reference: "clientId", name: "Client ID", value: "public-value", scope: "project",
    });
