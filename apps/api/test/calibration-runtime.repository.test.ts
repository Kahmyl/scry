...
import { CalibrationRuntimeRepository, type CalibrationRuntime } from "../src/calibration-runtime.repository.js";
...
function harness(responses: Array<{ rowCount: number; rows: unknown[] }>) {
  const client = { query: vi.fn(async (_text: string, _values?: unknown[]) => responses.shift() ?? { rowCount: 1, rows: [] }) };
  const database = {
    query: vi.fn(),
...
describe("calibration runtime lifecycle", () => {
  it("atomically enters running with an explicitly typed JSON phase", async () => {
    ...
    expect(String(client.query.mock.calls[0]![0])).toContain("jsonb_build_object('phase',$3::text)");
  ...
  it("terminalizes a claimed setup failure without permitting a retry after mutation", async () => {
    ...
    await expect(repository.failClaim(runtime, "CALIBRATION_WORKER_EXECUTION_FAILED", "42P18")).resolves.toBe(true);
  ...
  it("recovers stale claims with retained attempt diagnostics and an append-only event", async () => {
    const { repository, client } = harness([
      { rowCount: 1, rows: [{ sessionId: runtime.sessionId, attemptId: runtime.attemptId, mutationCount: 0 }] },
      { rowCount: 1, rows: [] },
      { rowCount: 1, rows: [] },
    ]);
