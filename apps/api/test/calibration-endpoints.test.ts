...
const calibration = {
  ...
  decide: vi.fn(async (_principal, _calibrationId, _attestationId, decision) => ({
    route: decision,
  })),
...
const missionContext = {
  ...
  agentSessionId: ids.agentSession,
};
...
describe("calibration HTTP contract", () => {
  ...
  beforeAll(async () => {
    ...
    app = await NestFactory.create<NestFastifyApplication>(CalibrationEndpointTestModule, adapter, {
      logger: false,
    });
  ...
  it("routes every calibration lifecycle endpoint through its authoritative service", async () => {
    const request = await call("POST", `/api/projects/${ids.project}/calibration-sessions`, {
      ...missionContext,
      name: "Synthetic credential calibration",
      sourceFlowRevisionId: ids.revision,
      operationId: "capture-secret",
      environmentId: ids.environment,
      disposableDataConfirmed: true,
      confirmedUserAuthorized: true,
      purpose: "Verify the complete calibration lifecycle",
      idempotencyKey: "request-unique-1",
    });
    expect(request.statusCode).toBe(201);
    expect(calibration.request).toHaveBeenCalledOnce();

    expect((await call("GET", `/api/projects/${ids.project}/calibrations`)).json()).toEqual({
      route: "list",
    });
    expect((await call("GET", `/api/calibrations/${ids.calibration}`)).json()).toEqual({
      route: "get",
    });
    expect(
      (
        await call(
          "POST",
          `/api/calibrations/${ids.calibration}/attestations/${ids.attestation}/approve`,
          { ...missionContext, confirmedUserAuthorized: true, reasonCode: "VERIFIED" },
        )
      ).json(),
    ).toEqual({ route: "approved" });
    expect(
      (
        await call(
          "POST",
          `/api/calibrations/${ids.calibration}/attestations/${ids.attestation}/reject`,
          { ...missionContext, reasonCode: "STRUCTURAL_DRIFT" },
        )
      ).json(),
    ).toEqual({ route: "rejected" });
    expect(
      (
        await call("POST", `/api/calibration-sessions/${ids.session}/retry`, {
          ...missionContext,
          idempotencyKey: "retry-unique-1",
        })
      ).json(),
    ).toEqual({ route: "retry" });
    expect(
      (
        await call("POST", `/api/calibration-sessions/${ids.session}/cancel`, missionContext)
      ).json(),
    ).toEqual({ route: "cancel" });
    expect(
      (
        await call("POST", `/api/flows/${ids.flow}/calibration-bindings`, {
          ...missionContext,
          ...
          idempotencyKey: "binding-unique-1",
        })
      ).json(),
    ).toEqual({ route: "bind" });
  });
  ...
  it("rejects malformed caller-authored calibration input before service execution", async () => {
    ...
    const response = await call("POST", `/api/projects/${ids.project}/calibration-sessions`, {
      ...
      disposableDataConfirmed: true,
      confirmedUserAuthorized: true,
      purpose: "Attempt to inject attestation output",
      idempotencyKey: "unsafe-unique-1",
