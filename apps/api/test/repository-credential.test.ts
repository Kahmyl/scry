...
const context = { missionId:"55555555-5555-4555-8555-555555555555",objectiveId:"66666666-6666-4666-8666-666666666666",agentSessionId:"77777777-7777-4777-8777-777777777777" };
...
describe("project credential creation", () => {
  it("encrypts the supplied value and returns metadata only", async () => {
    ...
    const created = await repository.createCredential(principal, "44444444-4444-4444-8444-444444444444", {
      ...context,
      name: "Preview password",
  ...
  it("returns a typed conflict instead of overwriting an existing credential", async () => {
    ...
    await expect(repository.createCredential(principal, "44444444-4444-4444-8444-444444444444", {
      ...context,
      name: "Preview password",
      ...
      response: {
        ...
        message: "An active credential with this name already exists. Existing credentials cannot be overwritten.",
      },
    });
  });
});
