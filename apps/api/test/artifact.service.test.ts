...
function admission(id: string, content: Uint8Array) {
  ...
  return {
    schemaVersion: 1 as const, evidenceId: id, channel: "dom" as const, classification: "public" as const,
    disposition: "sanitize" as const, policyDigest: "a".repeat(64), decisionId: "decision-1",
    contentDigest: digest, omissionIntervals: [], createdAt: "2026-08-03T00:00:00.000Z",
  };
}

describe("ArtifactService", () => {
  const key = "test-admission-key-that-is-at-least-32-bytes";
  const sanitation = { stage: "post_capture", method: "SecretRedactor.redact", attestedAt: "2026-08-03T00:00:00.000Z" };
  ...
  it("refuses retrieval as soon as retention destruction is claimed", async () => {
    process.env.VEIL_ADMISSION_KEY = key;
    const repository = { getArtifact: vi.fn().mockResolvedValue({ id: "claimed", availability: "available", destructionStatus: "deleting", storageKey: "run/claimed", contentType: "text/plain" }) };
    await expect(new ArtifactService(repository as never).range({ kind: "service", subject: "scry-service" }, "claimed")).rejects.toThrow("Artifact is not available");
  });
  ...
  it("pages, searches, extracts, and ranges large text artifacts", async () => {
    ...
    const repository = { getArtifact: vi.fn().mockResolvedValue({
      id: "11111111-1111-4111-8111-111111111111", availability: "available", destructionStatus: "pending", storageKey: "run/docs.html", contentType: "text/html",
      checksumSha256: proof.contentDigest, observation: { veilManifest: proof, veilSanitation: sanitation, veilAdmissionToken: signVeilEvidenceAdmission(proof, key, sanitation).token },
    }) };
    const service = new ArtifactService(repository as never);
  ...
  it("extracts the complete nested element instead of stopping at the first nested closing tag", async () => {
    ...
    const repository = { getArtifact: vi.fn().mockResolvedValue({
      id: "22222222-2222-4222-8222-222222222222", availability: "available", destructionStatus: "pending", storageKey: "run/nested.html", contentType: "text/html",
      checksumSha256: proof.contentDigest, observation: { veilManifest: proof, veilSanitation: sanitation, veilAdmissionToken: signVeilEvidenceAdmission(proof, key, sanitation).token },
    }) };
    const service = new ArtifactService(repository as never);
