...
const preferencesBaseline = expandFoundation(
  baselineSource.replace("\\ir veil-artifact-retention.sql\n", ""),
)
  .replace("\\ir authoring-execution-cutover.sql", () => authoringCutover)
  .replace("\\ir praxis-reporting.sql", () => praxisReporting)
  .replace("\\ir praxis-cutoff.sql", () => praxisCutoff)
  ...
  if (!baselineTable.rows[0]?.table) {
    ...
    if (
      ...
      try {
        ...
        if (installedFingerprint !== preferencesFingerprint)
        ...
        await client.query(
          "UPDATE schema_baseline SET schema_fingerprint=$1, applied_at=now() WHERE singleton=true",
          [fingerprint],
        );
        await client.query("COMMIT");
      ...
      throw new Error(
        `Schema fingerprint mismatch: installed=${installed.rows[0]?.schema_fingerprint ?? "missing"} required=${fingerprint}. Stop all writers and run the guarded capability-grounding pre-production cutover; compatibility migration is intentionally unsupported.`,
      );
    } else {
...
async function backfillVeilPolicySnapshots(client: pg.PoolClient) {
  ...
  for (const run of runs.rows) {
    ...
    await client.query(`UPDATE runs SET veil_policy_snapshot=$2::jsonb WHERE id=$1`, [
      run.id,
