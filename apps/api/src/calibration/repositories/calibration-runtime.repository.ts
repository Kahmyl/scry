...
import { decryptCredential } from "../../access/index.js";
...
export class CalibrationRuntimeRepository {
  ...
  ): Promise<CalibrationRuntime | undefined> {
    return this.database.transaction(async (client) => {
      ...
      const attempt = await client.query<{ attemptNumber: number }>(
        `SELECT COALESCE(max(attempt_number),0)+1 AS "attemptNumber"
         FROM calibration_attempts WHERE session_id=$1`,
        [sessionId],
      );
      ...
      await client.query(
        `UPDATE calibration_sessions SET state='claimed',current_attempt_id=$2,updated_at=now() WHERE id=$1`,
        [sessionId, attemptId],
      );
      ...
      const runtime = await client.query<
        Omit<CalibrationRuntime, "sessionId" | "attemptId" | "claimToken"> & {
          operation: ProtectedTransaction;
        }
      >(
        `SELECT revision.id AS "contractRevisionId",revision.operation_digest AS "operationDigest",
         ...
         WHERE revision.id=$1`,
        [session.rows[0]!.contractRevisionId],
      );
    ...
    return this.database.transaction(async (client) => {
      const result = await client.query(
        `UPDATE calibration_attempts SET state='running',heartbeat_at=now(),safe_diagnostics=jsonb_build_object('phase',$3::text)
         WHERE id=$1 AND claim_token=$2 AND state='claimed' RETURNING id`,
        [runtime.attemptId, runtime.claimToken, phase],
      );
      if (!result.rowCount) return false;
      const session = await client.query(
        `UPDATE calibration_sessions SET state=$2::text,updated_at=now() WHERE id=$1 AND current_attempt_id=$3 RETURNING id`,
        [runtime.sessionId, phase, runtime.attemptId],
      );
  ...
  async heartbeat(runtime: CalibrationRuntime) {
    const result = await this.database.query(
      `UPDATE calibration_attempts SET heartbeat_at=now() WHERE id=$1 AND claim_token=$2 AND state='running' RETURNING id`,
  ...
  async markPhase(runtime: CalibrationRuntime, phase: string) {
    return this.database.transaction(async (client) => {
      ...
      const session = await client.query(
        `UPDATE calibration_sessions SET state=$2,updated_at=now() WHERE id=$1 AND current_attempt_id=$3 RETURNING id`,
        [runtime.sessionId, phase, runtime.attemptId],
      );
      if (!session.rowCount) throw new Error("CALIBRATION_SESSION_OWNERSHIP_LOST");
      await this.appendEvent(
  ...
  async complete(runtime: CalibrationRuntime, completion: CalibrationCompletion) {
    return this.database.transaction(async (client) => {
      ...
      if (
        ...
        completion.mutationCount === 1 &&
        completion.privacyVerified &&
        completion.canaryScanPassed
      ) {
        ...
        await client.query(
          ...
          [
            attestationId,
            runtime.contractRevisionId,
            runtime.attemptId,
            runtime.operationDigest,
            runtime.inputSchemaDigest,
            runtime.inputDigest,
            completion.fingerprint,
            JSON.stringify(completion.structure),
            JSON.stringify(completion.protectionResult ?? { status: "passed" }),
            JSON.stringify(completion.extractionResult ?? { status: "passed" }),
            JSON.stringify(completion.safeExitResult ?? { status: "passed" }),
            runtime.claimToken,
          ],
        );
        await client.query(
          `UPDATE calibration_attempts SET state='attested',mutation_state='completed',mutation_count=1,safe_diagnostics=$3::jsonb,completed_at=now(),heartbeat_at=now() WHERE id=$1 AND claim_token=$2`,
          [runtime.attemptId, runtime.claimToken, JSON.stringify(completion.diagnostics)],
        );
        await client.query(
          `UPDATE calibration_sessions SET state='attested',completed_at=now(),updated_at=now() WHERE id=$1 AND current_attempt_id=$2`,
          [runtime.sessionId, runtime.attemptId],
        );
        await this.appendEvent(
          client,
          runtime.sessionId,
      ...
      await client.query(
        ...
        [
          ...
          runtime.claimToken,
          terminal,
          completion.mutationCount > 0 ? "unknown" : "not_started",
          completion.mutationCount,
          completion.failureProvenance ?? "infrastructure",
          String(completion.diagnostics.code ?? "CALIBRATION_FAILED"),
          JSON.stringify(completion.diagnostics),
        ],
      );
      await client.query(
        `UPDATE calibration_sessions SET state=$3,completed_at=now(),updated_at=now() WHERE id=$1 AND current_attempt_id=$2`,
        [runtime.sessionId, runtime.attemptId, terminal],
      );
      await this.appendEvent(
        client,
        runtime.sessionId,
        ...
        terminal,
        String(completion.diagnostics.code ?? "CALIBRATION_FAILED"),
        { stepId: completion.diagnostics.stepId, failureProvenance: completion.failureProvenance },
      );
      return false;
    });
  }
    ...
    const result = await this.database.query<{
      ...
      `SELECT credential.ciphertext,credential.initialization_vector AS "initializationVector",credential.authentication_tag AS "authenticationTag"
       ...
       WHERE credential.id=$2 AND credential.project_id=$3 AND credential.deleted_at IS NULL AND credential.security_status='active'
         AND environment.secret_refs ? ($2::uuid)::text`,
      [runtime.environmentId, credentialId, runtime.projectId],
    );
    ...
    return this.database.transaction(async (client) => {
      const stale = await client.query<{
        ...
        `UPDATE calibration_attempts SET state=CASE WHEN mutation_count=0 THEN 'failed' ELSE 'mutation_outcome_unknown' END,
         ...
         RETURNING session_id AS "sessionId",id AS "attemptId",mutation_count AS "mutationCount"`,
        [staleBefore],
      );
      for (const item of stale.rows) {
        const terminal = item.mutationCount === 0 ? "failed" : "mutation_outcome_unknown";
        await client.query(
          `UPDATE calibration_sessions SET state=$2,completed_at=now(),updated_at=now() WHERE id=$1 AND current_attempt_id=$3`,
          [item.sessionId, terminal, item.attemptId],
        );
        await this.appendEvent(
          client,
          item.sessionId,
