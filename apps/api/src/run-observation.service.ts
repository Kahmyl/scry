...
export class RunObservationService {
  ...
  async observe(principal: Principal, runId: string): Promise<RunObservation> {
      ...
      [
        ...
        this.database.query(
          `SELECT attempt_id AS "attemptId", step_id AS "stepId", ordinal,
             ...
             FROM step_results WHERE attempt_id = $1 ORDER BY ordinal`,
          [currentAttemptId],
        ),
        this.database.query(
          `SELECT attempt_id AS "attemptId", step_id AS "stepId", assertion_index AS "assertionIndex",
             ...
             FROM assertion_results WHERE attempt_id = $1 ORDER BY step_id, assertion_index`,
          [currentAttemptId],
        ),
        this.database.query(
          `SELECT id, attempt_id AS "attemptId", sequence, type, payload, occurred_at AS "occurredAt"
             FROM run_events WHERE attempt_id = $1 ORDER BY sequence`,
          [currentAttemptId],
        ),
        this.database.query(
          `SELECT id, attempt_id AS "attemptId", step_id AS "stepId", kind, availability,
             ...
             FROM artifacts WHERE attempt_id = $1 ORDER BY created_at, id`,
          [currentAttemptId],
        ),
        this.database.query(
          `SELECT id, sequence, entry_type AS type, artifact_id AS "artifactId", operation_id AS "operationId",
             ...
             FROM artifact_timeline_entries WHERE attempt_id = $1 ORDER BY sequence`,
          [currentAttemptId],
        ),
        this.database.query(
          `SELECT id, operation_id AS "operationId", sequence, mode, started_at AS "startedAt", ended_at AS "endedAt",
             ...
             FROM privacy_intervals WHERE attempt_id = $1 ORDER BY sequence`,
          [currentAttemptId],
        ),
        this.database.query(
          `SELECT transaction.operation_id AS "operationId",
             ...
             WHERE transaction.run_id = $1 ORDER BY transaction.created_at`,
          [runId],
        ),
        this.database.query(
          `SELECT id, credential_id AS "credentialId", operation_id AS "operationId", adapter_id AS "adapterId",
             ...
             FROM credential_incidents WHERE run_id = $1 ORDER BY created_at`,
          [runId],
        ),
      ]);
      const [transactionRows, findingRows] = await Promise.all([
        this.database.query(
          `SELECT transaction_id AS "transactionId",operation_id AS "operationId",step_id AS "stepId",schema_version AS "schemaVersion",runtime_version AS "runtimeVersion",result,started_at AS "startedAt",completed_at AS "completedAt" FROM praxis_transactions WHERE attempt_id=$1 ORDER BY started_at,transaction_id`,
          [currentAttemptId],
        ),
        this.database.query(
          `SELECT finding.id,finding.transaction_id AS "transactionId",finding.step_id AS "stepId",finding.intent_digest AS "intentDigest",finding.finding,finding.artifact_refs AS "artifactRefs",finding.created_at AS "createdAt" FROM praxis_quality_findings finding JOIN praxis_transactions transaction ON transaction.transaction_id=finding.transaction_id WHERE transaction.attempt_id=$1 ORDER BY finding.created_at,finding.id`,
          [currentAttemptId],
        ),
      ]);
    ...
    const planSteps = Array.isArray(run.planSnapshot?.steps)
      ? (run.planSnapshot.steps as Array<{ id?: string; title?: string }>)
      : [];
    ...
    for (const assertion of assertionRows.rows) {
      ...
      list.push({
        index: assertion.assertionIndex,
    ...
    const steps = stepRows.rows.map((step) => ({
      ...
      stepId: step.stepId,
      title:
        planSteps.find((candidate) => candidate.id === step.stepId)?.title ?? String(step.stepId),
    ...
    const integrity = inspectIntegrity(
      run,
      currentAttempt,
      planSteps,
      steps,
    ...
    const section = (items: unknown[]) =>
      items.length ? ("complete" as const) : active ? ("pending" as const) : ("complete" as const);
    const safeActions: RunObservation["safeActions"] = [];
    if (active) safeActions.push("cancel");
    else safeActions.push("rerun");
    if (failure && ["plan", "product"].includes(failure.provenance))
      safeActions.push("revise_flow");
    if (artifacts.some((artifact) => artifact.availability === "available"))
      safeActions.push("read_artifact");
    ...
    return runObservationSchema.parse({
      ...
      privacy: {
        ...
        credentialIncidents: incidents.rows,
      },
      veil,
      praxis: {
        contractVersion: 1,
        runtimeVersions: [
          ...new Set(praxisTransactions.map((transaction) => String(transaction.runtimeVersion))),
        ],
        status: active ? "pending" : "complete",
        transactions: praxisTransactions.map((transaction) => ({
          ...transaction,
          stepId: transaction.stepId ?? null,
          startedAt: new Date(transaction.startedAt).toISOString(),
          completedAt: transaction.completedAt
            ? new Date(transaction.completedAt).toISOString()
            : null,
        })),
        findings: praxisFindings.map((finding) => ({
          ...finding,
      ...
      sections: {
        ...
        steps: section(steps),
        events: section(events.rows),
        artifacts: section(artifacts),
        timeline: section(artifactTimeline),
      },
      integrity,
      safeActions,
      release: {
...
function deriveVeilObservation(
  ...
  const withheld = artifacts.filter(
    (artifact) =>
      ["quarantined", "destroyed", "failed", "incomplete"].includes(
        String(artifact.availability),
      ) && artifact.reasonCode,
  );
  const normalizedTimeline: VeilRunObservation["timeline"] = [
    ...intervals.map((entry, index) => ({
      sequence: Number(entry.sequence ?? index),
      type: entry.mode === "protected_recording_gap" ? ("gap" as const) : ("transition" as const),
      startedAt: iso(entry.startedAt),
      ...(entry.endedAt ? { endedAt: iso(entry.endedAt) } : {}),
      reasonCode: safeReason(entry.failureCode ?? entry.terminalState ?? "VEIL_PROTECTED_INTERVAL"),
    })),
    ...timeline
      .filter(
        (entry) => entry.type === "quarantine_record" || entry.type === "unavailable_interval",
      )
      .map((entry, index) => ({
        sequence: intervals.length + Number(entry.sequence ?? index),
        type: entry.type === "unavailable_interval" ? ("gap" as const) : ("disposition" as const),
        startedAt: iso(entry.startedAt),
        ...(entry.endedAt ? { endedAt: iso(entry.endedAt) } : {}),
        reasonCode: safeReason(entry.reasonCode ?? "VEIL_EVIDENCE_WITHHELD"),
        ...(artifactKindChannel(entry.channel)
          ? { channel: artifactKindChannel(entry.channel)! }
          : {}),
      })),
  ...
  const sealed =
    ...
    gapRows.some((entry) => !entry.endedAt);
  return {
    schemaVersion: 1,
...
function safeReason(value: unknown): string {
  const normalized = String(value ?? "VEIL_UNSPECIFIED")
    .toUpperCase()
    .replace(/[^A-Z0-9_]/g, "_");
...
function remediationFor(reason: unknown): string {
  ...
  if (code.includes("PERMIT") || code.includes("POLICY"))
    return "Start a new capture under the current Veil policy and document epoch.";
  if (code.includes("SEALED") || code.includes("GATE_CLOSED"))
    return "Inspect the preceding Veil transition, restore a safe context, and rerun the affected step.";
  return "Review the Veil timeline and rerun only after the affected evidence channel reports a verified-safe state.";
}
function artifactKindChannel(
  kind: unknown,
): VeilRunObservation["findings"][number]["channel"] | undefined {
  return ["screenshot", "video", "dom", "network", "trace"].includes(String(kind))
    ? (String(kind) as VeilRunObservation["findings"][number]["channel"])
    : undefined;
}
...
function deriveFailure(
...
): RunObservation["failure"] {
  if (!attempt && !terminalStates.has(String(run.state))) return null;
  const failedStep = steps.find(
    (step) =>
      step.action.status === "failed" ||
      step.readiness?.status === "failed" ||
      step.assertions.some((assertion: Record<string, any>) => assertion.status === "failed"),
  );
  const failedAssertion = failedStep?.assertions.find(
    (assertion: Record<string, any>) => assertion.status === "failed",
  );
  const policy = [...events]
    .reverse()
    .find(
      (event) =>
        event.type === "policy.rejected" && event.payload?.disposition !== "blocked_subresource",
    );
  const classification = String(run.outcomeClassification ?? "");
  if (!failedStep && !attempt?.error && !policy && run.state === "passed") return null;
  const provenance =
    classification === "infrastructure_failure" || run.state === "infrastructure_error"
      ? "infrastructure"
      : classification === "policy_failure" || policy
        ? "policy"
        : classification.includes("assertion") || classification === "confirmed_product_failure"
          ? "product"
          : classification === "cancelled" || classification === "execution_timeout"
            ? "executor"
            : "plan";
  const readinessError =
    failedStep?.readiness && typeof failedStep.readiness === "object"
  ...
  const message =
    ...
    attempt?.error ??
    policy?.payload?.message;
  return {
    provenance,
    code: String(
      policy?.payload?.code ?? (classification || `${String(run.state).toUpperCase()}_FAILURE`),
    ),
    ...(message ? { message: String(message) } : {}),
    ...(failedStep ? { stepId: String(failedStep.stepId) } : {}),
    ...(failedStep?.action.status === "failed"
      ? { channel: "action" }
      : failedStep?.readiness?.status === "failed"
        ? { channel: "readiness" }
        : failedAssertion
          ? { channel: "assertion" }
          : {}),
...
function inspectIntegrity(
  ...
  artifacts: Array<Record<string, any>>,
  timeline: Array<Record<string, any>>,
): RunObservation["integrity"] {
  const issues: Array<{ code: string; message: string }> = [];
  const terminal = terminalStates.has(String(run.state));
  if (terminal && !attempt)
    issues.push({
      code: "TERMINAL_ATTEMPT_MISSING",
      message: "The terminal run has no persisted attempt.",
    });
  if (terminal && attempt && !attempt.completedAt)
    issues.push({
      code: "TERMINAL_ATTEMPT_INCOMPLETE",
      message: "The terminal attempt has no completion timestamp.",
    });
  if (
    terminal &&
    planSteps.length > 0 &&
    ...
    issues.push({
      code: "TERMINAL_STEP_RESULTS_MISSING",
      message: "The terminal run has no persisted step results.",
    });
  const artifactIds = new Set(artifacts.map((artifact) => String(artifact.id)));
  for (const entry of timeline)
    if (entry.artifactId && !artifactIds.has(String(entry.artifactId)))
      issues.push({
        code: "TIMELINE_ARTIFACT_MISSING",
        message: `Timeline entry ${String(entry.id)} references an unavailable artifact manifest record.`,
      });
  const timelineArtifactIds = new Set(
    timeline
  ...
  for (const artifact of artifacts)
    if (
      ...
      ["video", "trace"].includes(String(artifact.kind)) &&
      !timelineArtifactIds.has(String(artifact.id))
    )
      issues.push({
        code: "AVAILABLE_ARTIFACT_TIMELINE_MISSING",
        message: `Available ${String(artifact.kind)} artifact ${String(artifact.id)} is absent from the artifact timeline.`,
      });
