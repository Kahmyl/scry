...
export class OrchestrationService implements OnModuleInit, OnModuleDestroy {
  ...
  private async scheduleProject(c: PoolClient, missionId: string, sessionId: string, requested?: string[]) {
    ...
    if (!slots) return [];
    const ready = await c.query<any>(`SELECT x.objective_id,b.flow_revision_id,b.compiled_contract_id,b.environment_id,b.execution_settings,fr.plan,e.name,e.base_origin,e.policy,e.secret_refs,fc.compiled_contract_digest FROM mission_objective_orchestration x JOIN mission_execution_plans ep ON ep.id=x.plan_id AND ep.status='active' JOIN mission_execution_bindings b ON b.plan_id=x.plan_id AND b.objective_id=x.objective_id JOIN flow_revisions fr ON fr.id=b.flow_revision_id JOIN environments e ON e.id=b.environment_id JOIN flow_compilations fc ON fc.id=b.compiled_contract_id AND fc.flow_revision_id=fr.id AND fc.environment_id=e.id AND fc.status='execution_ready' WHERE x.mission_id=$1 AND x.state='ready' AND b.mode='automatic' AND ($2::uuid[] IS NULL OR x.objective_id=ANY($2)) ORDER BY x.updated_at,x.objective_id FOR UPDATE OF x SKIP LOCKED`, [missionId, requested?.length ? requested : null]);
    ...
    for (const row of ready.rows) {
      ...
      const run = await c.query<{ id: string }>(`INSERT INTO runs(project_id,mission_id,objective_id,agent_session_id,environment_id,flow_revision_id,compiled_contract_id,compiled_contract_digest,state,phase,plan_snapshot,environment_snapshot,policy_snapshot,execution_snapshot,idempotency_key) VALUES($1,$2,$3,$4,$5,$6,$7,$8,'queued','queued',$9,$10,$11,$12,$13) RETURNING id`, [project.rows[0]!.project_id, missionId, row.objective_id, sessionId, row.environment_id, row.flow_revision_id,row.compiled_contract_id,row.compiled_contract_digest, row.plan, JSON.stringify({ id: row.environment_id, name: row.name, baseOrigin: row.base_origin, policy: row.policy, secretRefs: row.secret_refs }), row.policy, row.execution_settings, idempotencyKey]);
