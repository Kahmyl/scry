  ...
  async search(principal: Principal, artifactId: string, query: string, maxMatches = 20) {
    ...
    while (offset < size && matches.length < Math.min(maxMatches, 100)) {
      ...
      while ((index = text.indexOf(query, index)) >= 0 && matches.length < Math.min(maxMatches, 100)) {
        const absolute = Math.max(0, offset - Buffer.byteLength(carry) + Buffer.byteLength(text.slice(0, index)));
        matches.push({ offset: absolute, context: text.slice(Math.max(0, index - 120), index + query.length + 120) });
...
function matchesSelector(element: HtmlElement, selector: string) {
  ...
  if (selector.startsWith(".")) return (attribute(element, "class") ?? "").split(/\s+/).includes(selector.slice(1));
  if (selector.startsWith("[")) return attribute(element, "data-testid") === selector.match(/["']([^"']+)["']/)![1];
