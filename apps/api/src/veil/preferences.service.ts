...
export class VeilPreferencesService {
  ...
  async get(principal: Principal, environmentId: string): Promise<VeilPreferenceRecord> {
    ...
    const preferences =
      environment.preferences ?? defaultPreferences(environment.policy.allowedOrigins);
  ...
  async tighten(
    ...
    raw: VeilPreferenceUpdate,
  ): Promise<VeilPreferenceRecord> {
    ...
    const effectivePolicy = compileVeilPolicy([
      snapshotPreferences(current.effectivePolicy),
      desired,
    ]);
...
function assertTightening(current: VeilPolicySnapshot, desired: VeilPolicySnapshot) {
  const addsOrigin = desired.allowedOrigins.some(
    (origin) => !current.allowedOrigins.includes(origin),
  );
  const enablesChannel = Object.entries(desired.controls).some(
    ([key, value]) => value && !current.controls[key as keyof typeof current.controls],
  );
  ...
  if (addsOrigin || enablesChannel || desired.leaseTtlMs > current.leaseTtlMs || weakensProfile) {
    throw new BadRequestException({
      ...
      message:
        "Veil preferences may only remove origins, disable capture channels, or shorten lease duration.",
    });
  }
}
