...
import {
  ...
  type CalibrationStructure,
  type MutationLedgerState,
  type ProtectedTransactionStore,
} from "@scry/executor";
...
import type {
  ...
  CalibrationRuntime,
} from "./calibration-runtime.repository.js";
...
export async function runCalibrationAttestation(
  ...
  const transactionStore: ProtectedTransactionStore = {
    claim: async ({ operationId }) => ({
      state: states.get(operationId) ?? "planned",
      fencingToken: 1,
    }),
  ...
  try {
    const targetIndex = runtime.plan.steps.findIndex(
      (step) =>
        step.action.type === "protectedTransaction" &&
        step.action.operationId === runtime.operationId,
    );
    if (targetIndex < 0)
      return {
        passed: false,
        ...
        canaryScanPassed: false,
        failureProvenance: "plan",
        diagnostics: { code: "CALIBRATION_OPERATION_MISMATCH", phase: "preparing" },
      };
    const calibrationSteps = runtime.plan.steps.slice(0, targetIndex + 1);
    const calibrationPlan = {
      ...runtime.plan,
      steps: calibrationSteps.map((step, index) =>
        index === targetIndex && step.action.type === "protectedTransaction"
          ? {
              ...step,
              action: {
                ...step.action,
                continuation: { strategies: [{ mode: "terminal" as const }] },
              },
            }
          : step,
      ),
    ...
    const report = await executePlan({
      ...
      atomicSecretCapture: async ({ value }) => {
        capturedProtectedValues.push(value);
        return { credentialId: crypto.randomUUID() };
      },
      publicValueCapture: async ({ reference, value }) => {
        const valueId = crypto.randomUUID();
        capturedPublicValues.set(valueId, value);
        capturedPublicValues.set(reference, value);
        return { valueId };
      },
      ...
      onEvent: (event) => {
        if (event.type === "privacy.operation_failed")
          protectedFailure = safeProtectedFailure(event.payload);
        if (
          event.type === "privacy.operation_completed" &&
          event.payload.result &&
          typeof event.payload.result === "object"
        )
          transactionResult = event.payload.result as Record<string, unknown>;
      },
    });
    ...
    if (
      !runtime.plan.allowedOrigins
        .map((value) => new URL(value).origin)
        .includes(new URL(boundary.url).origin)
    ) {
      return {
        passed: false,
        mutationCount,
        privacyVerified: false,
        canaryScanPassed: false,
        failureProvenance: "policy",
        diagnostics: { code: "CALIBRATION_ORIGIN_MISMATCH", phase: "boundary_reached" },
      };
    }
    const leakChannels = capturedProtectedValues.length
      ? [
          ...new Set(
            (
              await Promise.all(
                capturedProtectedValues.map((value) =>
                  canaryLeakChannels(report, outputDirectory, value),
                ),
              )
            ).flat(),
          ),
    ...
    return {
      ...
      canaryScanPassed,
      diagnostics: {
        code: canaryScanPassed ? "CALIBRATION_ATTESTED" : "CALIBRATION_CANARY_DETECTED",
        phase: "scanning_channels",
        mutationCount,
        ...(leakChannels.length ? { leakChannels } : {}),
      },
      protectionResult: { status: "passed", mode: "separate_browser_capsule" },
      extractionResult: {
        status: "passed",
        outputCount: runtime.operation.extraction.outputs.length,
      },
    ...
    return {
      ...
      privacyVerified: false,
      canaryScanPassed: false,
      failureProvenance: "infrastructure",
      diagnostics: {
        code: safeCode(error),
        phase: boundary ? "executing_protected_transaction" : "executing_preflight",
      },
    };
  } finally {
...
async function scanDirectory(root: string, value: string, channels: Set<string>): Promise<void> {
  for (const entry of await readdir(root, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      await scanDirectory(target, value, channels);
      continue;
    }
    const bytes = await readFile(target);
    if (
      containsCanary(Buffer.from(entry.name), value) ||
      containsCanary(bytes, value) ||
      (entry.name.endsWith(".zip") && zipContainsCanary(bytes, value))
    ) {
      channels.add(
...
function zipContainsCanary(bytes: Buffer, value: string) {
  try {
    return Object.entries(unzipSync(bytes)).some(
      ([name, contents]) =>
        containsCanary(Buffer.from(name), value) || containsCanary(Buffer.from(contents), value),
    );
  } catch {
    return false;
  }
...
function containsCanary(bytes: Buffer, value: string) {
  ...
  return [...new Set(variants)].some(
    (variant) => variant.length > 0 && bytes.includes(Buffer.from(variant)),
  );
}
...
function safePhase(result: Record<string, unknown> | undefined, boundary: unknown) {
  const value = result?.failurePhase;
...
function safeReason(result: Record<string, unknown> | undefined) {
  const value = result?.reasonCode;
  return typeof value === "string" && /^[A-Z][A-Z0-9_]*$/.test(value)
    ? value
    : "CALIBRATION_TRANSACTION_FAILED";
}
function safeRetry(result: Record<string, unknown> | undefined) {
  const value = result?.retryClass;
  return typeof value === "string" &&
    ["safe_to_retry", "retry_requires_reconciliation", "do_not_retry", "manual_review"].includes(
      value,
    )
    ? value
    : "manual_review";
  ...
  return diagnostics
    .map((item) => {
      ...
      return {
        ...
        containerResolved: Boolean(value.containerResolved),
        matchCount: safeEnum(value.matchCount, ["none", "one", "many", "unknown"]),
        ...
        /^[A-Z][A-Z0-9_]*$/.test(value.lastFailureCode)
  ...
  return effects
    .map((item) => {
      ...
      const method =
        ...
        ["POST", "PUT", "PATCH", "DELETE", "OTHER"].includes(value.method)
          ? value.method
          : "OTHER";
      ...
      const category =
        value.category === "telemetry" || value.category === "platform"
          ? value.category
          : undefined;
...
function safeProtectedFailure(payload: Record<string, unknown>) {
  ...
  return {
    code:
      typeof payload.code === "string" && /^[A-Z][A-Z0-9_]*$/.test(payload.code)
        ? payload.code
        : "PROTECTED_TRANSACTION_FAILED",
    status:
      typeof payload.status === "string" && /^[a-z_]+$/.test(payload.status)
        ? payload.status
        : "failed",
    ...(typeof payload.stage === "string" && /^[a-z_]+$/.test(payload.stage)
      ? { stage: payload.stage }
      : {}),
    ...(typeof payload.boundaryVerified === "boolean"
      ? { boundaryVerified: payload.boundaryVerified }
      : {}),
    ...(typeof payload.knownValueCleared === "boolean"
      ? { knownValueCleared: payload.knownValueCleared }
      : {}),
    diagnostics,
  };
}
