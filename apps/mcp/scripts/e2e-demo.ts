...
const project =
  projects[0] ??
  (await api.post<{ id: string; name: string }>("/projects", {
...
try {
  ...
  const started = await call("start_mission", {
    ...
    originalInstruction:
      "Verify Scry health through the complete Mission-aware MCP execution path.",
  ...
  const createdObjective = await call("create_mission_objective", {
    ...
    dependencies: [],
    completionCriteria: [
      { description: "The API health document visibly reports status ok.", required: true },
    ],
    order: 0,
  });
  ...
  if ((validation.validation as Record<string, unknown>).valid !== true)
    throw new Error(`Execution plan rejected: ${JSON.stringify(validation.validation)}`);
  ...
  for (let poll = 0; poll < 120; poll += 1) {
    ...
    const orchestration = (await call("get_orchestration_status", { missionId }))
      .orchestration as Record<string, unknown>;
    objective = (orchestration.objectives as Record<string, unknown>[]).find(
    ...
    if (
      ["passed", "failed", "cancelled", "timed_out", "infrastructure_error"].includes(currentState)
    )
      break;
  }
  ...
  if (state !== "passed")
    throw new Error(
      `MCP Run did not pass: ${JSON.stringify({ runId, state, failure: observation.failure })}`,
    );
  ...
  await call("accept_objective_evidence", {
    ...
    artifactIds: [],
    conclusion:
      "The Scry landing page rendered the expected primary heading in a controlled Chromium Run.",
  });
