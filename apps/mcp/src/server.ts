...
export function createScryMcpServer(client = new ScryApiClient()) {
  ...
  server.registerTool("get_grounding_diagnostics",{title:"Get semantic grounding diagnostics",description:"Read sanitized target confidence, ambiguity, drift, and permitted next actions for a Run.",inputSchema:{runId:uuid},annotations:readOnly},async({runId})=>result({diagnostics:await client.get(`/runs/${runId}/grounding`)},"Sanitized grounding diagnostics loaded."));
